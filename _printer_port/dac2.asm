;*********************************************
;* WilliamTell.z80
;*********************************************
;* Description:
;*  Plays the William Tell Overture through a DAC
;*  attached to the Laser/VZ parallel port.
;*  A direct translation from the 6809 code to the Z80.
;*  Code order altered from 6809 version.
;*  Original code published in Color Computer News July 1982 Issue
;*
;* Author:  James Diffendaffer (z80 version)
;*
;* To Do:
;*  Test with real DAC hardware
;*  Adjust wave/note/song playback speed for VZ & Z80
;*  Switch hex notation from remaining Motorola leading $ to Zilog trailing h
;*
;* NOTES:
;*  Assembles with tasm3.1 assembler.  Build VZ file with following commands.
;*	tasm31 -80 -b WilliamTell.z80 WilliamTell.obj
;*  copy WilliamTell.obj will.obj
;*  rbinary will.obj will.vz
;*  
;* Version History:
;*  1.2 - 10/21/2020 Converted note table to little endian, code fixes, additional comments added
;*  1.1 - 09/19/2010 Corrected assembler command line in header and changed play code
;*  1.0 - 08/17/2010 Cleaned up the code, order doesn't match 6809 code anymore, added macros and fixed errors in comments.
;*  0.9 - 08/17/2010 Revised code to automatically align wave table on 256 byte memory page.
;*  0.8 - 08/16/2010 Revised header and added unverified music data.
;*  0.7 - 08/16/2010 Saving the duration had been left out of READ and fixed first line of music data from dump listing,
;*                   added macro in NOTE 
;*  0.6	- 08/16/2010 Cleaned up header content, added comments
;*  0.5 - 08/16/2010 Aligned waveform table on 256 byte boundary. Made additional little endian fixes to note/wave play code
;*	0.4 - 08/03/2010 Altered voice pointer code for little endian in the note/wave play routine
;*  0.3 - 08/02/2010 Fixed tests for status bits not being set with instructions used, optimized some code
;*  0.2 - 08/02/2010 save registers on entry, restore on exit
;*  0.1 - 08/01/2010 Initial Build
;*********************************************



; vz definitions we need
VZExit	equ 1A19h	; Address to jump to when exiting a snapshot file
PORT	equ 14		; output port for DAC
TEMPO	equ $30


	org	$8000			;start for VZ snapshot


START	
	
MUSIC	di
	push	hl			; save registers we modify
	push	bc
	push	af
	push	ix
	push	iy
	
; INITIALIZATION
	ld		hl,ORGAN	; point to ORGAN waveform table with HL. All channels are using the same waveform for this demo.  
						; Each channel or note could use a different instrument with a new music file format
	ld		(VOICE1+1),hl	; set wave table for channel 1.  Note increment doesn't impact most significant byte.
	ld		(VOICE2+1),hl	;  chennel 2
	ld		(VOICE3+1),hl	;  channel 3
	ld		(VOICE4+1),hl	;  channel 4
;	note increment least significant byte is initialized by the assembler as zero by the definition below
;	we could do the same with the two bytes of the voice poiter if we wanted to

;	ld		hl,MSTRT	; get music data start
;	ld		(MUWORK),hl	; init pointer to current song location being read
	ld		ix,MSTRT	; put address of music data in ix, no need to save it
LOOP
	;* READ NOTE DURATION
;	ld		ix,(MUWORK)	; get current pointer to song -removed as it's in IX for the entire song
	ld		a,(ix)		; get note duration from song
	cp		$0			; is duration zero?
	jr		z,QUIT		; if so, we are done playing and we QUIT
	ld		(DUR),a		; if not, set duration
	inc		ix			; point to next byte

	call	NOTE		; get notes to play
	call	PLAY		; play it
	jp		LOOP		; continue looping
QUIT
	pop		iy
	pop		ix
	pop		af
	pop		bc
	pop		hl
	jp		VZExit		; exit snapshot


;************************************
;* NOTE
;************************************
;* PROCEDURE TO GET THE NEXT NOTES FOR THE FOUR VOICES
;************************************
NOTE
	ld		b,0			; clear b

	ld	c,(ix)		; get note
	inc	ix			; increment note pointer
	ld	iy,TABL		; point to note table
	add	iy,bc		; add note offset
	ld	l,(iy)		; get lsb of note wavefore increment
	ld	h,(iy+1)	; get msb of note wavefore increment
	ld	(INC1),hl		; save it to channel increment

	ld	c,(ix)		; get note
	inc	ix			; increment note pointer
	ld	iy,TABL		; point to note table
	add	iy,bc		; add note offset
	ld	l,(iy)		; get lsb of note wavefore increment
	ld	h,(iy+1)	; get msb of note wavefore increment
	ld	(INC2),hl		; save it to channel increment

	ld	c,(ix)		; get note
	inc	ix			; increment note pointer
	ld	iy,TABL		; point to note table
	add	iy,bc		; add note offset
	ld	l,(iy)		; get lsb of note wavefore increment
	ld	h,(iy+1)	; get msb of note wavefore increment
	ld	(INC3),hl		; save it to channel increment

	ld	c,(ix)		; get note
	inc	ix			; increment note pointer
	ld	iy,TABL		; point to note table
	add	iy,bc		; add note offset
	ld	l,(iy)		; get lsb of note wavefore increment
	ld	h,(iy+1)	; get msb of note wavefore increment
	ld	(INC4),hl		; save it to channel increment
	ret					;end note

	
;***********************************************
;* PLAY
;***********************************************
;* PROCEDURE TO PLAY A NOTE
;***********************************************
PLAY
	ld		b,TEMPO
PLAY1
	;add current 4 voice samples together to mix them
	ld		hl,(VOICE1+1)
	ld		a,(hl)
	ld		hl,(VOICE2+1)
	add		a,(hl)
	ld		hl,(VOICE3+1)
	adc	a,(hl)			; switched to addc to match original code
	ld		hl,(VOICE4+1)
	adc	a,(hl)			; switched to addc to match original code
					; removed, the CoCo's 6 bit DAC drops the two bottom bits, not the two top bits
;	rrca					; shift the bits down 2 to match the CoCo's 6 bit DAC
;	rrca
;	and		$3F				; mask off the bits that rotated around

	out		(PORT),a		; output to D/A converter (parallel port latch)
;	out		(PORT-1),a		; output to D/A converter (parallel port strobe)


	ld	hl,(VOICE1)
	ld	de,(INC1)
	add	hl,de
	ld	(VOICE1),hl

	ld	hl,(VOICE2)
	ld	de,(INC2)
	add	hl,de
	ld	(VOICE2),hl

	ld	hl,(VOICE3)
	ld	de,(INC3)
	add	hl,de
	ld	(VOICE3),hl

	ld	hl,(VOICE4)
	ld	de,(INC4)
	add	hl,de
	ld	(VOICE4),hl


	djnz	TIMEWAS			; dec & check tempo, jp if not done
	
	ld		hl,DUR
	dec		(hl)			; decrement duration
	jp		nz,PLAY
ENDNOT	ret						;end play


TIMEWAS	jp		WASTE1		; waste some CPU cycles
WASTE1	jp		WASTE2
WASTE2	jp		WASTE3
WASTE3	jp		WASTE4
WASTE4	jp		WASTE5
WASTE5	jp		WASTE6
WASTE6	jp		WASTE7
WASTE7	jp		WASTE8
WASTE8	jp		PLAY1			;continue playing


; variables are stored here
;MUWORK	DB	0,0	; current music data pointer.  Removed as it is now stored in IX
DUR		DB	0	; duration
VOICE1	DB	0,0,0	; VOICE1 current waveform pointer with offset is VOICE1+1, 1st byte is used as part of the count
VOICE2	DB	0,0,0	; VOICE2 ...
VOICE3	DB	0,0,0	; VOICE3 ...
VOICE4	DB	0,0,0	; VOICE4 ...
INC1	DB	0,0		; current wave increment to use for VOICE1 in playback loop (these impact frequency)
INC2	DB	0,0
INC3	DB	0,0
INC4	DB	0,0

	;to align WFT on a 256 byte boundary with TASM assembler
	ORG  (($ + 0FFH) & 0FF00H)
;*
;* WAVEFORM TABLE FOR ORGAN SOUND (256 bytes)
;*
ORGAN
	DB	$0D,$0E,$0F,$11,$12,$14,$15,$16 
	DB	$18,$19,$1B,$1C,$1D,$1F,$20,$22 
	DB	$23,$26,$27,$29,$2A,$2B,$2D,$2E 
	DB	$30,$31,$32,$34,$35,$37,$38,$38 
	DB	$39,$3B,$3B,$3C,$3C,$3E,$3E,$3E 
	DB	$3F,$3F,$3F,$3F,$3F,$3F,$3F,$3E 
	DB	$3E,$3E,$3C,$3C,$3B,$3B,$39,$38 
	DB	$37,$37,$35,$34,$32,$31,$30,$2E 
	DB	$2E,$2D,$2B,$2A,$29,$27,$26,$24 
	DB	$23,$22,$20,$1F,$1F,$1D,$1C,$1B 
	DB	$1B,$19,$19,$18,$18,$16,$16,$15 
	DB	$15,$15,$15,$15,$15,$15,$15,$15 
	DB	$15,$15,$15,$15,$16,$16,$16,$18 
	DB	$18,$18,$19,$19,$1B,$1B,$1C,$1C 
	DB	$1C,$1D,$1D,$1F,$1F,$1F,$20,$20 
	DB	$22,$22,$22,$22,$22,$23,$23,$23 
	DB	$23,$23,$23,$23,$22,$22,$22,$22 
	DB	$20,$20,$20,$1F,$1F,$1D,$1D,$1C 
	DB	$1C,$1B,$19,$19,$18,$16,$15,$15
	DB	$14,$12,$12,$11,$0F,$0E,$0E,$0D 
	DB	$0B,$0B,$0A,$08,$08,$07,$07,$06 
	DB	$06,$04,$04,$03,$03,$03,$01,$01
	DB	$01,$01,$01,$01,$00,$00,$00,$00
	DB	$00,$01,$01,$01,$01,$01,$01,$01
	DB	$03,$03,$03,$03,$03,$04,$04,$04
	DB	$04,$04,$06,$06,$06,$06,$06,$06 
	DB	$06,$06,$07,$07,$07,$07,$07,$07 
	DB	$07,$06,$06,$06,$06,$06,$06,$06 
	DB	$06,$06,$04,$04,$04,$04,$04,$04 
	DB	$04,$04,$04,$04,$03,$03,$03,$04 
	DB	$04,$04,$04,$04,$04,$06,$06,$06 
	DB	$07,$07,$07,$08,$0A,$0A,$0B,$0D

;*
;* NOTE TABLE
;*
TABL
;	little endian version
;	contains the 16 bit step rate to advance throught the waveform data for different notes
	DB	$00,$00,$6F,$02,$94,$02,$E8,$02 
	DB	$E4,$02,$10,$03,$3F,$03,$71,$03
	DB	$E4,$02,$10,$03,$3F,$03,$71,$03
	DB	$A5,$03,$DC,$03,$17,$04,$56,$04
	DB	$98,$04,$0D,$04,$28,$05,$76,$05 
	DB	$C9,$05,$21,$06,$7F,$06,$E2,$06 
	DB	$4A,$07,$89,$07,$2F,$08,$AC,$08 
	DB	$30,$09,$BB,$09,$50,$0A,$ED,$0A 
	DB	$93,$0B,$43,$0C,$FE,$0C,$C4,$00 
	DB	$95,$0E,$73,$0F				; A ABOVE MIDDLE C (440 HERTZ)?
	DB	$5F,$10,$58,$11,$60,$12,$77,$13
	DB	$A0,$14,$DA,$15,$26,$17,$87,$18
	DB	$FC,$19,$88,$1B,$2B,$1D,$E7,$1E
	DB	$BE,$20,$B0,$22,$C0,$24,$EF,$26
	DB	$40,$29,$B4,$2B,$4D,$2E,$0E,$31
	DB	$F9,$33,$10,$37,$57,$3A,$CF,$3D
	DB	$7C,$41,$60,$45,$81,$49,$DF,$4D
	DB	$81,$52,$69,$57,$9B,$5C,$1D,$62

;	big endian version
;	contains the step rate to advance throught the waveform data for different notes
;	DB	$00,$00,$02,$6F,$02,$94,$02,$E8 
;	DB	$02,$E4,$03,$10,$03,$3F,$03,$71
;	DB	$03,$A5,$03,$DC,$04,$17,$04,$56 
;	DB	$04,$98,$04,$0D,$05,$28,$05,$76 
;	DB	$05,$C9,$06,$21,$06,$7F,$06,$E2 
;	DB	$07,$4A,$07,$89,$08,$2F,$08,$AC 
;	DB	$09,$30,$09,$BB,$0A,$50,$0A,$ED 
;	DB	$0B,$93,$0C,$43,$0C,$FE,$00,$C4 
;	DB	$0E,$95,$0F,$73				; A ABOVE MIDDLE C (440 HERTZ)?
;	DB	$10,$5F,$11,$58,$12,$60,$13,$77
;	DB	$14,$A0,$15,$DA,$17,$26,$18,$87
;	DB	$19,$FC,$1B,$88,$1D,$2B,$1E,$E7
;	DB	$20,$BE,$22,$B0,$24,$C0,$26,$EF
;	DB	$29,$40,$2B,$B4,$2E,$4D,$31,$0E
;	DB	$33,$F9,$37,$10,$3A,$57,$3D,$CF
;	DB	$41,$7C,$45,$60,$49,$81,$4D,$DF
;	DB	$52,$81,$57,$69,$5C,$9B,$62,$1D

;****************	
MSTRT	;music...
;121B
	DB	24,0,0,0,10
;*******************************************************************************
;*                                                                             *
;*      Notes for 'William Tell Overture' by Gioachino Rossini                 *
;*       Notes transcribed from July 1982 Color Computer News                  *
;*       Credits to Garry and Linda Howard                                     *
;*                                                                             *
;*      note format: note_duration,freq_v1,freq_v2,freq_v3,freq_v4             *
;*                                                                             *
;*******************************************************************************

NoteStart
                db     $18,$00,$00,$00,$0A
                db     $06,$00,$00,$00,$08
                db     $06,$00,$00,$00,$0A
                db     $06,$00,$00,$00,$10
                db     $06,$00,$00,$00,$18
                db     $06,$00,$00,$00,$22
                db     $06,$00,$00,$00,$28
                db     $06,$00,$00,$00,$30
                db     $06,$00,$00,$00,$3A
                db     $0C,$40,$00,$00,$00
                db     $09,$00,$00,$00,$00
                db     $03,$40,$00,$00,$00
                db     $30,$48,$00,$00,$00
                db     $18,$48,$3A,$28,$22
                db     $18,$48,$38,$2C,$26
                db     $18,$48,$3A,$28,$22
                db     $12,$48,$40,$38,$18
                db     $04,$48,$3E,$38,$18
                db     $01,$00,$00,$00,$00
                db     $0C,$48,$3E,$38,$18
                db     $0C,$48,$00,$00,$00
                db     $12,$4A,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $01,$00,$00,$00,$00
                db     $24,$48,$00,$00,$00
                db     $24,$00,$00,$00,$00
                db     $18,$0E,$00,$00,$00
                db     $06,$0C,$00,$00,$00
                db     $06,$0E,$00,$00,$00
                db     $06,$14,$00,$00,$00
                db     $06,$18,$00,$00,$00
                db     $06,$20,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $0C,$3E,$00,$00,$00
                db     $09,$00,$00,$00,$00
                db     $03,$3E,$00,$00,$00
                db     $30,$48,$00,$00,$00
                db     $18,$48,$3E,$30,$2C
                db     $18,$48,$3A,$30,$28
                db     $18,$48,$3E,$30,$20
                db     $12,$48,$44,$30,$22
                db     $04,$48,$40,$30,$22
                db     $01,$00,$00,$00,$00
                db     $0C,$48,$40,$30,$22
                db     $0C,$48,$00,$00,$00
                db     $12,$4A,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $01,$00,$00,$00,$00
                db     $24,$48,$00,$00,$00
                db     $24,$00,$00,$00,$00
                db     $18,$28,$00,$22,$1A
                db     $0C,$00,$28,$1E,$18
                db     $06,$00,$2C,$1E,$18
                db     $06,$00,$30,$1E,$18
                db     $12,$3A,$32,$1A,$14
                db     $06,$3E,$32,$1A,$14
                db     $18,$40,$32,$1E,$10
                db     $0C,$40,$30,$1E,$10
                db     $0C,$00,$00,$00,$00
                db     $18,$3C,$32,$24,$14
                db     $0C,$30,$2C,$20,$18
                db     $0C,$00,$00,$00,$00
                db     $18,$3C,$00,$00,$00
                db     $18,$3C,$32,$24,$14
                db     $0C,$30,$2C,$20,$18
                db     $0C,$00,$00,$00,$00
                db     $18,$3E,$00,$00,$00
                db     $18,$3E,$32,$24,$14
                db     $0C,$3E,$38,$24,$18
                db     $0C,$3C,$00,$00,$00
                db     $0C,$3E,$00,$00,$00
                db     $0C,$3C,$00,$00,$00
                db     $0C,$3E,$00,$00,$00
                db     $0C,$3C,$00,$00,$00
                db     $0C,$3E,$00,$00,$00
                db     $0C,$32,$00,$00,$00
                db     $0C,$34,$00,$00,$00
                db     $0C,$32,$00,$00,$00
                db     $0C,$34,$00,$00,$00
                db     $0C,$30,$00,$00,$00
                db     $0C,$42,$30,$3A,$0A
                db     $0C,$42,$30,$3A,$22
                db     $0C,$42,$30,$3A,$0A
                db     $0C,$42,$30,$3A,$22
                db     $0C,$42,$30,$3A,$0A
                db     $0C,$42,$30,$3A,$22
                db     $0C,$44,$3A,$04,$00
                db     $06,$44,$3A,$1C,$00
                db     $06,$42,$3A,$1C,$00
                db     $0C,$44,$3A,$04,$00
                db     $06,$44,$3A,$1C,$00
                db     $06,$48,$3A,$1C,$00
                db     $0C,$4C,$44,$34,$14
                db     $0C,$4C,$44,$34,$14
                db     $0C,$48,$3E,$38,$18
                db     $0C,$48,$3E,$38,$18
                db     $08,$48,$3E,$38,$18
                db     $04,$4C,$3E,$38,$18
                db     $04,$4C,$3E,$38,$18
                db     $08,$48,$3E,$38,$18
                db     $08,$44,$36,$30,$18
                db     $08,$42,$36,$30,$18
                db     $08,$3E,$36,$30,$18
                db     $0C,$3A,$2A,$0A,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3A,$30,$12,$00
                db     $06,$3E,$30,$12,$00
                db     $06,$42,$30,$12,$00
                db     $0C,$3E,$38,$30,$18
                db     $0C,$3E,$38,$30,$18
                db     $0C,$42,$36,$0A,$00
                db     $0C,$42,$36,$22,$00
                db     $0C,$42,$30,$0A,$00
                db     $0C,$42,$30,$22,$00
                db     $0C,$42,$3A,$0A,$00
                db     $0C,$42,$3A,$22,$00
                db     $0C,$44,$3A,$04,$00
                db     $06,$44,$3A,$1C,$00
                db     $06,$48,$3A,$1C,$00
                db     $0C,$4C,$3A,$14,$00
                db     $0C,$4C,$3A,$14,$00
                db     $03,$22,$16,$00,$00
                db     $03,$3A,$16,$00,$00
                db     $03,$22,$16,$00,$00
                db     $03,$3A,$16,$00,$00
                db     $03,$22,$16,$00,$00
                db     $03,$3A,$16,$00,$00
                db     $03,$22,$16,$00,$00
                db     $03,$3A,$16,$00,$00
                db     $24,$22,$00,$00,$00
                db     $24,$3A,$00,$00,$00
                db     $0C,$30,$2A,$22,$18
                db     $0C,$3A,$2A,$22,$18
                db     $0C,$38,$2A,$22,$18
                db     $0C,$3A,$2A,$22,$18
                db     $0C,$3E,$2A,$22,$18
                db     $0C,$3A,$2A,$22,$18
                db     $12,$3A,$2C,$26,$18
                db     $06,$38,$2C,$26,$18
                db     $0C,$3E,$2C,$26,$18
                db     $0C,$48,$2C,$26,$18
                db     $0C,$4A,$2C,$26,$18
                db     $0C,$48,$2C,$26,$18
                db     $0C,$48,$30,$2A,$18
                db     $0C,$48,$3A,$2A,$18
                db     $0C,$48,$38,$2A,$18
                db     $0C,$00,$3A,$2A,$18
                db     $0C,$00,$3E,$2A,$18
                db     $0C,$00,$3A,$2A,$18
                db     $0C,$48,$3A,$26,$18
                db     $06,$4A,$3A,$26,$18
                db     $06,$4A,$38,$26,$18
                db     $0C,$4C,$38,$26,$18
                db     $0C,$48,$38,$26,$18
                db     $0C,$46,$38,$26,$18
                db     $0C,$44,$38,$26,$18
                db     $0C,$42,$3C,$34,$04
                db     $0C,$42,$3C,$34,$1C
                db     $0C,$42,$3C,$34,$04
                db     $0C,$42,$3C,$34,$1C
                db     $0C,$42,$3C,$34,$04
                db     $0C,$42,$3C,$34,$1C
                db     $0C,$44,$3E,$34,$0E
                db     $06,$44,$3E,$34,$26
                db     $06,$42,$3E,$34,$26
                db     $0C,$44,$3E,$34,$0E
                db     $06,$44,$3E,$34,$0E
                db     $06,$48,$3E,$34,$0E
                db     $0C,$4C,$44,$34,$0E
                db     $0C,$4C,$44,$34,$26
                db     $0C,$48,$3E,$38,$18
                db     $0C,$48,$3E,$38,$18
                db     $08,$48,$3E,$38,$18
                db     $08,$4C,$3E,$38,$18
                db     $08,$48,$3E,$38,$18
                db     $08,$44,$38,$30,$18
                db     $08,$42,$38,$30,$18
                db     $08,$3E,$38,$30,$18
                db     $0C,$3A,$2A,$04,$00
                db     $0C,$3A,$2A,$1C,$00
                db     $0C,$3A,$2E,$0E,$00
                db     $06,$3E,$2E,$0E,$00
                db     $06,$42,$2E,$0E,$00
                db     $0C,$3E,$38,$30,$18
                db     $0C,$3E,$38,$30,$18
                db     $0C,$42,$36,$30,$0C
                db     $0C,$42,$36,$30,$24
                db     $0C,$42,$3A,$30,$0A
                db     $0C,$42,$3A,$30,$22
                db     $0C,$42,$3C,$30,$06
                db     $0C,$42,$3C,$30,$1E
                db     $0C,$44,$3A,$32,$02
                db     $06,$44,$3A,$32,$1A
                db     $06,$48,$3A,$32,$1A
                db     $0C,$4A,$3A,$32,$14
                db     $0C,$4A,$00,$00,$00
                db     $18,$3C,$32,$2C,$24
                db     $0C,$40,$3A,$32,$1A
                db     $06,$40,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $0C,$32,$00,$00,$00
                db     $24,$00,$00,$00,$00
                db     $12,$3A,$30,$2A,$22
                db     $06,$30,$30,$2A,$22
                db     $0C,$2A,$00,$00,$22
                db     $24,$00,$00,$00,$00
                db     $18,$00,$00,$00,$26
                db     $06,$00,$00,$00,$24
                db     $06,$00,$00,$00,$26
                db     $06,$00,$00,$00,$2C
                db     $06,$00,$00,$00,$34
                db     $06,$00,$00,$00,$3C
                db     $06,$00,$00,$00,$3E
                db     $06,$00,$00,$00,$44
                db     $06,$00,$00,$00,$42
                db     $12,$3E,$3C,$2E,$00
                db     $06,$3E,$38,$30,$00
                db     $0C,$3E,$38,$30,$00
                db     $24,$3E,$00,$00,$00
                db     $12,$3E,$34,$2C,$00
                db     $06,$3E,$32,$28,$00
                db     $0C,$3E,$32,$32,$00
                db     $24,$3E,$00,$00,$00
                db     $12,$3C,$32,$2C,$20
                db     $06,$3C,$30,$2A,$20
                db     $0C,$3C,$32,$2C,$20
                db     $24,$3C,$00,$00,$00
                db     $18,$3E,$30,$20,$18
                db     $18,$3E,$2E,$20,$18
                db     $14,$3E,$2C,$20,$18
                db     $02,$3E,$3A,$20,$18
                db     $02,$3E,$3E,$20,$18
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3A,$36,$30,$00
                db     $0C,$3A,$36,$30,$0A
                db     $0C,$3A,$34,$2E,$00
                db     $0C,$3A,$34,$2E,$0A
                db     $0C,$3A,$38,$32,$2C
                db     $0C,$3A,$30,$2A,$0A
                db     $0C,$30,$22,$1E,$00
                db     $0C,$30,$22,$1E,$0A
                db     $0C,$2E,$22,$1C,$00
                db     $0C,$2E,$22,$26,$0A
                db     $0C,$2C,$22,$1A,$00
                db     $0C,$2A,$22,$18,$0A
                db     $0C,$00,$18,$12,$0A
                db     $06,$3E,$00,$12,$0A
                db     $06,$40,$00,$12,$0A
                db     $06,$22,$12,$0A,$00
                db     $06,$28,$12,$0A,$00
                db     $06,$30,$18,$12,$0A
                db     $06,$3A,$18,$12,$0A
                db     $06,$42,$18,$12,$0A
                db     $06,$3E,$18,$12,$0A
                db     $0A,$3A,$18,$12,$0A
                db     $01,$00,$00,$00,$00
                db     $18,$3A,$18,$12,$0A
                db     $0A,$00,$18,$12,$0A
                db     $01,$00,$00,$00,$00
                db     $0A,$00,$18,$12,$0A
                db     $01,$00,$00,$00,$00
                db     $0C,$00,$18,$12,$0A
                db     $0A,$18,$00,$12,$0A
                db     $01,$00,$00,$00,$00
                db     $0C,$18,$00,$12,$0A
                db     $06,$16,$00,$12,$0A
                db     $06,$18,$00,$12,$0A
                db     $06,$22,$00,$12,$0A
                db     $06,$2A,$00,$12,$0A
                db     $06,$30,$18,$12,$0A
                db     $06,$3A,$18,$12,$0A
                db     $06,$42,$18,$12,$0A
                db     $06,$48,$18,$12,$0A
                db     $0C,$52,$18,$12,$0A
                db     $36,$52,$00,$00,$00
                db     $04,$52,$42,$3A,$0A
                db     $01,$00,$00,$00,$00
                db     $0C,$52,$42,$3A,$0A
                db     $0C,$52,$00,$00,$00
                db     $0C,$52,$42,$3A,$18
                db     $0C,$52,$00,$00,$00
                db     $0C,$52,$42,$3A,$12
                db     $0C,$52,$00,$00,$00
                db     $0C,$52,$42,$3A,$0A
                db     $0C,$00,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$36,$2A,$24,$1C
                db     $06,$34,$2A,$24,$1C
                db     $06,$30,$00,$24,$1C
                db     $06,$2C,$00,$24,$1C
                db     $06,$2A,$00,$24,$1C
                db     $06,$2C,$00,$24,$1C
                db     $06,$30,$00,$24,$1C
                db     $06,$34,$00,$24,$1C
                db     $0C,$36,$24,$1C,$00
                db     $24,$00,$00,$00,$00
                db     $18,$3C,$54,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$42,$5A,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$34,$4C,$00,$00
                db     $18,$00,$00,$00,$00
                db     $06,$48,$3E,$00,$00
                db     $06,$3E,$4C,$00,$00
                db     $06,$48,$3E,$00,$00
                db     $06,$3E,$4C,$00,$00
                db     $06,$48,$3E,$00,$00
                db     $06,$3E,$4C,$00,$00
                db     $06,$48,$3E,$00,$00
                db     $06,$3E,$4C,$00,$00
                db     $18,$48,$3E,$00,$00
                db     $18,$3E,$4C,$00,$00
                db     $18,$48,$3E,$00,$00
                db     $18,$3E,$4C,$00,$00
                db     $18,$48,$3E,$00,$00
                db     $18,$3E,$4C,$00,$00
                db     $0C,$48,$3E,$00,$00
                db     $24,$00,$00,$00,$00
                db     $18,$44,$2C,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$4C,$34,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$3E,$26,$00,$00
                db     $18,$00,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$36,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$26
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$26
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$26
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$26
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$26
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$30
                db     $06,$40,$34,$2E,$30
                db     $06,$3E,$34,$2E,$30
                db     $06,$40,$34,$2E,$30
                db     $06,$3E,$34,$2E,$30
                db     $06,$40,$34,$2E,$26
                db     $06,$3E,$34,$2E,$26
                db     $06,$3A,$00,$2E,$26
                db     $06,$36,$00,$2E,$26
                db     $06,$34,$00,$2E,$26
                db     $06,$36,$00,$2E,$26
                db     $06,$3A,$00,$2E,$26
                db     $06,$3E,$00,$2E,$26
                db     $06,$40,$00,$2E,$26
                db     $06,$3E,$00,$2E,$26
                db     $06,$3A,$00,$2E,$26
                db     $06,$36,$00,$2E,$26
                db     $06,$34,$00,$2E,$26
                db     $06,$36,$00,$2E,$26
                db     $06,$3A,$00,$2E,$26
                db     $06,$3E,$00,$2E,$26
                db     $0C,$40,$00,$2E,$26
                db     $24,$00,$00,$00,$00
                db     $18,$00,$00,$46,$00
                db     $18,$00,$00,$00,$00
                db     $18,$00,$4C,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$00,$00,$3E,$00
                db     $18,$00,$00,$00,$00
                db     $06,$52,$48,$00,$00
                db     $06,$48,$56,$00,$00
                db     $06,$52,$48,$00,$00
                db     $06,$48,$56,$00,$00
                db     $06,$52,$48,$00,$00
                db     $06,$48,$56,$00,$00
                db     $06,$52,$48,$00,$00
                db     $06,$48,$56,$00,$00
                db     $18,$52,$48,$00,$00
                db     $18,$48,$56,$00,$00
                db     $18,$52,$48,$00,$00
                db     $18,$48,$56,$00,$00
                db     $0C,$4E,$00,$00,$48
                db     $24,$00,$00,$00,$00
                db     $18,$4E,$00,$00,$36
                db     $18,$00,$00,$00,$00
                db     $18,$56,$3E,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$48,$30,$00,$00
                db     $18,$00,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$30
                db     $06,$48,$3E,$38,$30
                db     $06,$4A,$3E,$38,$1C
                db     $06,$48,$3E,$38,$1C
                db     $06,$44,$00,$38,$30
                db     $06,$40,$00,$38,$30
                db     $06,$3E,$00,$38,$30
                db     $06,$40,$00,$38,$30
                db     $06,$44,$00,$38,$30
                db     $06,$48,$00,$38,$30
                db     $06,$4A,$00,$38,$30
                db     $06,$48,$00,$38,$30
                db     $06,$44,$00,$38,$30
                db     $06,$40,$00,$38,$30
                db     $06,$3E,$00,$38,$30
                db     $06,$40,$00,$38,$30
                db     $06,$44,$00,$38,$30
                db     $06,$48,$00,$38,$30
                db     $0C,$4A,$36,$30,$00
                db     $24,$00,$00,$00,$00
                db     $18,$50,$38,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$56,$3E,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$48,$30,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$44,$4A,$00,$00
                db     $18,$3E,$52,$00,$00
                db     $18,$44,$4A,$00,$00
                db     $18,$3E,$52,$00,$00
                db     $18,$44,$4A,$00,$00
                db     $18,$3E,$52,$00,$00
                db     $18,$44,$4A,$00,$00
                db     $18,$3E,$52,$00,$00
                db     $0C,$44,$50,$48,$00
                db     $24,$00,$00,$00,$00
                db     $18,$44,$2C,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$4A,$32,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$3E,$26,$00,$00
                db     $18,$00,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$52,$32,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$56,$32,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$00,$4A,$32,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$52,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$56,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2E,$4C,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2E,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$34,$52,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$34,$56,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$4C,$2E,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$4C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$52,$34,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$56,$34,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$18,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$48,$32,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$48,$32,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$48,$32,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$40,$32,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$32,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$36,$32,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$36,$32,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$3A,$32,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$38,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$3E,$18,$00,$00
                db     $06,$40,$2C,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$44,$18,$00,$00
                db     $06,$3A,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$48,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$48,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$48,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$4E,$2C,$00,$00
                db     $06,$4A,$18,$00,$00
                db     $06,$44,$2C,$00,$00
                db     $06,$50,$18,$00,$00
                db     $06,$52,$2C,$00,$00
                db     $06,$50,$18,$00,$00
                db     $06,$52,$18,$00,$00
                db     $06,$50,$18,$00,$00
                db     $06,$52,$18,$00,$00
                db     $06,$50,$18,$00,$00
                db     $06,$48,$2C,$00,$00
                db     $0C,$58,$52,$0A,$10
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$50,$28,$22,$00
                db     $06,$50,$30,$00,$00
                db     $06,$4E,$28,$22,$00
                db     $06,$4E,$30,$00,$00
                db     $06,$4C,$28,$22,$00
                db     $06,$4C,$30,$00,$00
                db     $06,$4A,$28,$22,$00
                db     $06,$4A,$30,$00,$00
                db     $06,$48,$28,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$46,$28,$22,$00
                db     $06,$46,$30,$00,$00
                db     $06,$44,$28,$22,$00
                db     $06,$44,$30,$00,$00
                db     $06,$42,$28,$22,$5A
                db     $06,$42,$30,$00,$5A
                db     $06,$40,$28,$22,$58
                db     $06,$40,$30,$00,$58
                db     $06,$3E,$28,$22,$56
                db     $06,$3E,$30,$00,$56
                db     $06,$3C,$28,$22,$54
                db     $06,$3C,$30,$00,$54
                db     $0C,$52,$3A,$28,$22
                db     $0C,$00,$00,$00,$18
                db     $0C,$58,$52,$1A,$00
                db     $0C,$58,$52,$1C,$00
                db     $0C,$58,$52,$1E,$00
                db     $0C,$58,$52,$20,$00
                db     $0C,$58,$52,$22,$00
                db     $0C,$58,$52,$24,$00
                db     $0C,$58,$52,$28,$00
                db     $0C,$58,$52,$2C,$00
                db     $0C,$58,$52,$2E,$00
                db     $0C,$58,$52,$30,$00
                db     $0C,$58,$52,$32,$00
                db     $0C,$58,$52,$30,$00
                db     $0C,$58,$52,$28,$00
                db     $0C,$58,$52,$26,$00
                db     $0C,$58,$52,$0A,$10
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$58,$52,$28,$22
                db     $06,$58,$52,$30,$00
                db     $06,$56,$28,$22,$00
                db     $06,$56,$30,$00,$00
                db     $06,$54,$28,$22,$00
                db     $06,$54,$30,$00,$00
                db     $06,$52,$28,$22,$00
                db     $06,$52,$30,$00,$00
                db     $06,$50,$28,$22,$00
                db     $06,$50,$30,$00,$00
                db     $06,$4E,$28,$22,$00
                db     $06,$4E,$30,$00,$00
                db     $06,$4C,$28,$22,$00
                db     $06,$4C,$30,$00,$00
                db     $06,$4A,$28,$22,$00
                db     $06,$4A,$30,$00,$00
                db     $06,$48,$28,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$46,$28,$22,$00
                db     $06,$46,$30,$00,$00
                db     $06,$44,$28,$22,$00
                db     $06,$44,$30,$00,$00
                db     $06,$42,$28,$22,$00
                db     $06,$42,$30,$00,$00
                db     $0C,$40,$28,$22,$00
                db     $0C,$18,$00,$00,$00
                db     $0C,$58,$52,$1A,$00
                db     $0C,$58,$52,$1C,$00
                db     $0C,$58,$52,$1E,$00
                db     $0C,$58,$52,$20,$00
                db     $0C,$58,$52,$22,$00
                db     $0C,$58,$52,$26,$00
                db     $0C,$58,$52,$28,$00
                db     $0C,$58,$52,$2C,$00
                db     $0C,$58,$52,$2E,$00
                db     $0C,$58,$52,$30,$00
                db     $0C,$58,$52,$32,$00
                db     $0C,$58,$52,$30,$00
                db     $0C,$58,$52,$28,$00
                db     $0C,$58,$52,$22,$00
                db     $0C,$5C,$14,$22,$0A
                db     $06,$5C,$2C,$22,$00
                db     $06,$5C,$32,$00,$00
                db     $06,$5C,$2C,$22,$00
                db     $06,$5C,$32,$00,$00
                db     $06,$5C,$2C,$22,$00
                db     $06,$5C,$32,$00,$00
                db     $06,$5C,$2C,$22,$00
                db     $06,$5C,$32,$00,$00
                db     $06,$5A,$2C,$22,$00
                db     $06,$5A,$32,$00,$00
                db     $06,$58,$2C,$22,$00
                db     $06,$58,$32,$00,$00
                db     $06,$56,$2C,$22,$00
                db     $06,$56,$32,$00,$00
                db     $06,$54,$2C,$22,$00
                db     $06,$54,$32,$00,$00
                db     $06,$52,$2C,$22,$00
                db     $06,$52,$32,$00,$00
                db     $06,$50,$2C,$22,$00
                db     $06,$50,$32,$00,$00
                db     $06,$4E,$2C,$22,$00
                db     $06,$4E,$32,$00,$00
                db     $06,$4C,$2C,$22,$00
                db     $06,$4C,$32,$00,$00
                db     $06,$4A,$2C,$22,$00
                db     $06,$4A,$32,$00,$00
                db     $06,$48,$2C,$22,$00
                db     $06,$48,$32,$00,$00
                db     $06,$46,$2C,$22,$00
                db     $06,$46,$32,$00,$00
                db     $0C,$44,$2C,$22,$00
                db     $0C,$22,$32,$00,$00
                db     $0C,$4A,$44,$24,$00
                db     $0C,$4A,$44,$26,$00
                db     $0C,$4A,$44,$28,$00
                db     $0C,$4A,$44,$2A,$00
                db     $0C,$4A,$44,$2C,$00
                db     $0C,$4A,$44,$30,$00
                db     $0C,$4A,$44,$32,$00
                db     $0C,$4A,$44,$30,$00
                db     $0C,$4A,$44,$36,$00
                db     $0C,$4A,$44,$32,$00
                db     $0C,$4A,$44,$2C,$00
                db     $0C,$4A,$44,$2A,$00
                db     $0C,$4A,$44,$30,$00
                db     $0C,$4A,$44,$2C,$00
                db     $0C,$52,$4A,$14,$0A
                db     $06,$52,$4A,$2C,$22
                db     $06,$52,$4A,$32,$00
                db     $06,$52,$4A,$2C,$22
                db     $06,$52,$4A,$32,$00
                db     $06,$52,$4A,$2C,$22
                db     $06,$52,$4A,$32,$00
                db     $06,$52,$4A,$2C,$22
                db     $06,$52,$4A,$32,$00
                db     $06,$48,$2C,$22,$00
                db     $06,$48,$32,$00,$00
                db     $06,$46,$2C,$22,$00
                db     $06,$46,$32,$00,$00
                db     $06,$44,$2C,$22,$00
                db     $06,$44,$32,$00,$00
                db     $06,$5A,$42,$2C,$22
                db     $06,$5A,$42,$32,$00
                db     $06,$58,$40,$2C,$22
                db     $06,$58,$40,$32,$00
                db     $06,$56,$3E,$2C,$22
                db     $06,$56,$3E,$32,$00
                db     $06,$54,$3C,$2C,$22
                db     $06,$54,$3C,$32,$00
                db     $06,$52,$3A,$2C,$22
                db     $06,$52,$3A,$32,$00
                db     $06,$50,$38,$2C,$22
                db     $06,$50,$38,$32,$00
                db     $06,$4E,$36,$2C,$22
                db     $06,$4E,$36,$32,$00
                db     $06,$4C,$34,$2C,$22
                db     $06,$4C,$34,$32,$00
                db     $0C,$4A,$32,$2C,$22
                db     $0C,$00,$00,$00,$22
                db     $0C,$4A,$32,$24,$00
                db     $0C,$4A,$32,$26,$00
                db     $0C,$4A,$32,$28,$00
                db     $0C,$4A,$32,$2A,$00
                db     $0C,$4A,$32,$2C,$00
                db     $0C,$4A,$32,$30,$00
                db     $0C,$4A,$32,$00,$00
                db     $0C,$4A,$32,$30,$00
                db     $0C,$4A,$32,$36,$00
                db     $0C,$4A,$32,$32,$00
                db     $0C,$4A,$32,$2C,$00
                db     $0C,$4A,$32,$2A,$00
                db     $0C,$4A,$32,$30,$00
                db     $0C,$4A,$32,$2C,$00
                db     $0C,$50,$56,$18,$00
                db     $06,$50,$56,$20,$18
                db     $06,$50,$56,$30,$00
                db     $06,$50,$56,$20,$18
                db     $06,$50,$56,$30,$00
                db     $06,$50,$56,$20,$18
                db     $06,$50,$56,$30,$00
                db     $06,$50,$56,$20,$18
                db     $06,$50,$56,$30,$00
                db     $06,$4E,$26,$20,$18
                db     $06,$4E,$30,$00,$00
                db     $06,$4C,$26,$20,$18
                db     $06,$4C,$30,$00,$00
                db     $06,$4A,$26,$20,$18
                db     $06,$4A,$30,$00,$00
                db     $06,$48,$26,$16,$18
                db     $06,$48,$30,$00,$00
                db     $06,$46,$26,$16,$18
                db     $06,$46,$30,$00,$00
                db     $06,$44,$26,$16,$18
                db     $06,$44,$30,$00,$00
                db     $06,$42,$26,$16,$18
                db     $06,$42,$30,$00,$00
                db     $06,$40,$26,$16,$18
                db     $06,$40,$30,$00,$00
                db     $06,$3E,$26,$16,$22
                db     $06,$3E,$30,$00,$00
                db     $06,$3C,$26,$16,$22
                db     $06,$3C,$30,$00,$00
                db     $06,$3A,$26,$16,$22
                db     $06,$3A,$30,$00,$00
                db     $0C,$50,$38,$20,$18
                db     $0C,$00,$00,$00,$18
                db     $0C,$50,$00,$00,$1A
                db     $0C,$50,$00,$00,$1C
                db     $0C,$50,$00,$00,$1E
                db     $0C,$50,$00,$00,$20
                db     $0C,$50,$00,$00,$22
                db     $0C,$50,$00,$00,$24
                db     $0C,$50,$26,$00,$00
                db     $0C,$50,$24,$00,$00
                db     $0C,$50,$28,$00,$00
                db     $0C,$50,$26,$00,$00
                db     $0C,$50,$22,$00,$00
                db     $0C,$50,$20,$00,$00
                db     $0C,$50,$26,$00,$00
                db     $0C,$50,$20,$00,$00
                db     $0C,$56,$18,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$56,$30,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$56,$30,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$56,$30,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$56,$30,$00,$00
                db     $06,$54,$26,$20,$18
                db     $06,$54,$30,$00,$00
                db     $06,$52,$26,$20,$18
                db     $06,$52,$30,$00,$00
                db     $06,$50,$26,$20,$18
                db     $06,$50,$30,$00,$00
                db     $06,$4E,$26,$20,$18
                db     $06,$4E,$30,$00,$00
                db     $06,$4C,$26,$20,$18
                db     $06,$4C,$30,$00,$00
                db     $06,$4A,$26,$20,$18
                db     $06,$4A,$30,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$30,$00,$00
                db     $06,$46,$26,$20,$18
                db     $06,$46,$30,$00,$00
                db     $06,$44,$26,$20,$18
                db     $06,$44,$30,$00,$00
                db     $06,$42,$26,$20,$18
                db     $06,$42,$30,$00,$00
                db     $06,$40,$26,$20,$18
                db     $06,$40,$30,$00,$00
                db     $0C,$56,$3E,$20,$18
                db     $0C,$00,$00,$00,$18
                db     $0C,$56,$50,$00,$1A
                db     $0C,$56,$50,$00,$1C
                db     $0C,$56,$50,$00,$1E
                db     $0C,$56,$50,$00,$20
                db     $0C,$56,$50,$00,$22
                db     $0C,$56,$50,$00,$24
                db     $0C,$56,$50,$26,$00
                db     $0C,$56,$50,$28,$00
                db     $0C,$56,$50,$2A,$00
                db     $0C,$56,$50,$2C,$00
                db     $0C,$56,$50,$2E,$00
                db     $0C,$56,$50,$30,$00
                db     $0C,$56,$48,$34,$00
                db     $0C,$56,$48,$38,$00
                db     $0C,$58,$52,$48,$3A
                db     $0C,$3A,$22,$00,$00
                db     $0C,$38,$22,$00,$00
                db     $0C,$3A,$20,$00,$00
                db     $0C,$3C,$1E,$00,$00
                db     $0C,$3E,$1A,$00,$00
                db     $0C,$40,$18,$00,$00
                db     $0C,$42,$16,$00,$00
                db     $0C,$44,$14,$00,$00
                db     $0C,$4A,$14,$00,$00
                db     $0C,$4A,$32,$2C,$00
                db     $0C,$48,$32,$2C,$00
                db     $0C,$46,$32,$2C,$00
                db     $0C,$44,$32,$2C,$00
                db     $0C,$42,$32,$2C,$00
                db     $0C,$40,$32,$2C,$00
                db     $0C,$3E,$1E,$00,$00
                db     $0C,$36,$1E,$00,$00
                db     $0C,$34,$1E,$00,$00
                db     $0C,$36,$1C,$00,$00
                db     $0C,$38,$1A,$00,$00
                db     $0C,$3A,$18,$00,$00
                db     $0C,$3C,$14,$00,$00
                db     $0C,$3E,$12,$00,$00
                db     $0C,$40,$10,$00,$00
                db     $0C,$48,$10,$00,$00
                db     $0C,$48,$30,$28,$00
                db     $0C,$44,$30,$28,$00
                db     $0C,$42,$30,$28,$00
                db     $0C,$40,$30,$28,$00
                db     $0C,$3E,$30,$28,$00
                db     $0C,$3C,$30,$28,$00
                db     $0C,$3A,$1A,$00,$00
                db     $0C,$32,$1A,$00,$00
                db     $0C,$30,$1A,$00,$00
                db     $0C,$32,$18,$00,$00
                db     $0C,$34,$16,$00,$00
                db     $0C,$36,$14,$00,$00
                db     $0C,$3A,$12,$00,$00
                db     $0C,$3C,$10,$00,$00
                db     $0C,$3E,$0E,$00,$00
                db     $0C,$5C,$44,$0E,$00
                db     $0C,$5C,$44,$2C,$26
                db     $0C,$5A,$42,$2C,$26
                db     $0C,$58,$42,$2C,$26
                db     $0C,$56,$3E,$2C,$26
                db     $0C,$54,$3C,$2C,$26
                db     $0C,$52,$3A,$2C,$26
                db     $0C,$50,$38,$30,$18
                db     $0C,$48,$30,$18,$00
                db     $0C,$46,$30,$18,$00
                db     $0C,$48,$2E,$16,$00
                db     $0C,$4C,$2C,$14,$00
                db     $0C,$4E,$2A,$12,$00
                db     $0C,$52,$28,$10,$00
                db     $0C,$56,$26,$0E,$00
                db     $06,$58,$22,$0A,$00
                db     $06,$40,$22,$0A,$00
                db     $06,$58,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$58,$3A,$00,$00
                db     $06,$40,$3A,$00,$00
                db     $06,$58,$3A,$00,$00
                db     $06,$40,$3A,$00,$00
                db     $0C,$58,$3A,$00,$00
                db     $0C,$56,$3A,$00,$00
                db     $0C,$54,$3A,$00,$00
                db     $0C,$52,$3A,$00,$00
                db     $0C,$58,$36,$00,$00
                db     $0C,$56,$36,$00,$00
                db     $0C,$54,$36,$00,$00
                db     $0C,$52,$36,$00,$00
                db     $0C,$58,$36,$00,$00
                db     $0C,$56,$36,$00,$00
                db     $0C,$54,$36,$00,$00
                db     $0C,$52,$36,$00,$00
                db     $0C,$58,$3A,$34,$00
                db     $0C,$56,$3A,$34,$00
                db     $0C,$54,$3A,$34,$00
                db     $0C,$52,$3A,$34,$00
                db     $0C,$58,$3A,$32,$00
                db     $0C,$56,$3A,$32,$00
                db     $0C,$54,$3A,$32,$00
                db     $0C,$52,$3A,$32,$00
                db     $0C,$58,$3A,$30,$00
                db     $0C,$56,$3A,$30,$00
                db     $0C,$54,$3A,$30,$00
                db     $0C,$52,$3A,$30,$00
                db     $0C,$58,$3A,$2E,$00
                db     $0C,$56,$3A,$2E,$00
                db     $0C,$54,$3A,$2E,$00
                db     $0C,$52,$3A,$2E,$00
                db     $06,$58,$4C,$3A,$2C
                db     $06,$40,$00,$3A,$2C
                db     $06,$58,$4C,$40,$00
                db     $06,$40,$00,$00,$00
                db     $06,$58,$4C,$3A,$2C
                db     $06,$40,$00,$3A,$2C
                db     $06,$58,$4C,$40,$00
                db     $06,$40,$00,$00,$00
                db     $0C,$58,$4C,$3A,$2C
                db     $0C,$56,$4C,$40,$00
                db     $0C,$54,$4C,$3A,$2C
                db     $0C,$52,$4C,$40,$00
                db     $0C,$58,$4A,$3A,$2E
                db     $0C,$56,$4A,$40,$00
                db     $0C,$54,$4A,$3A,$2E
                db     $0C,$52,$4A,$40,$00
                db     $0C,$58,$4A,$3A,$2E
                db     $0C,$56,$4A,$40,$00
                db     $0C,$54,$4A,$3A,$2E
                db     $0C,$52,$4A,$40,$00
                db     $0C,$58,$48,$3A,$30
                db     $0C,$56,$48,$40,$00
                db     $0C,$54,$48,$3A,$30
                db     $0C,$52,$48,$40,$00
                db     $0C,$58,$46,$3A,$32
                db     $0C,$56,$46,$40,$00
                db     $0C,$54,$46,$3A,$32
                db     $0C,$52,$46,$40,$00
                db     $0C,$58,$48,$3A,$30
                db     $0C,$56,$48,$40,$00
                db     $0C,$54,$48,$3A,$30
                db     $0C,$52,$48,$40,$00
                db     $0C,$58,$4C,$3A,$2E
                db     $0C,$56,$4C,$40,$00
                db     $0C,$54,$4C,$3A,$2E
                db     $0C,$52,$4C,$40,$00
                db     $0C,$60,$48,$38,$30
                db     $0C,$00,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$26,$20,$18
                db     $06,$40,$26,$20,$18
                db     $06,$3E,$26,$20,$18
                db     $06,$3A,$26,$20,$18
                db     $06,$38,$26,$20,$18
                db     $06,$3A,$26,$20,$18
                db     $06,$3E,$26,$20,$18
                db     $06,$40,$26,$20,$18
                db     $06,$44,$26,$20,$18
                db     $06,$44,$2C,$18,$00
                db     $06,$00,$26,$20,$18
                db     $06,$00,$2C,$18,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$2C,$18,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$2C,$18,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$2C,$18,$00
                db     $06,$4A,$26,$20,$18
                db     $06,$4A,$2C,$18,$00
                db     $06,$4C,$26,$20,$18
                db     $06,$4C,$2C,$18,$00
                db     $06,$4E,$26,$20,$18
                db     $06,$4E,$2C,$18,$00
                db     $06,$50,$26,$20,$18
                db     $06,$50,$2C,$18,$00
                db     $06,$52,$26,$20,$18
                db     $06,$52,$2C,$18,$00
                db     $06,$54,$26,$20,$18
                db     $06,$54,$2C,$18,$00
                db     $06,$56,$26,$20,$18
                db     $06,$56,$2C,$18,$00
                db     $06,$58,$26,$20,$18
                db     $06,$58,$2C,$18,$00
                db     $06,$5A,$26,$20,$18
                db     $06,$5A,$2C,$18,$00
                db     $06,$5C,$26,$20,$18
                db     $06,$5C,$2C,$18,$00
                db     $06,$60,$26,$20,$18
                db     $06,$60,$2C,$18,$00
                db     $0C,$58,$28,$22,$18
                db     $0C,$00,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$40,$2E,$1A,$22
                db     $06,$3E,$2E,$1A,$22
                db     $06,$3A,$2E,$1A,$22
                db     $06,$36,$2E,$1A,$22
                db     $06,$34,$2E,$1A,$22
                db     $06,$36,$2E,$1A,$22
                db     $06,$3A,$2E,$1A,$22
                db     $06,$3E,$2E,$1A,$22
                db     $06,$40,$2E,$1A,$22
                db     $06,$3E,$2E,$1A,$22
                db     $06,$3A,$2E,$1A,$22
                db     $06,$36,$2E,$1A,$22
                db     $06,$34,$2E,$1A,$22
                db     $06,$36,$2E,$1A,$22
                db     $06,$3A,$2E,$1A,$22
                db     $06,$3E,$2E,$1A,$22
                db     $06,$40,$22,$18,$1C
                db     $06,$40,$2E,$18,$1C
                db     $06,$00,$22,$18,$1C
                db     $06,$00,$2E,$18,$1C
                db     $06,$46,$22,$18,$1C
                db     $06,$46,$2E,$18,$1C
                db     $06,$46,$22,$18,$1C
                db     $06,$46,$2E,$18,$1C
                db     $06,$46,$22,$18,$1C
                db     $06,$46,$2E,$18,$1C
                db     $06,$48,$22,$18,$1C
                db     $06,$48,$2E,$18,$1C
                db     $06,$4A,$22,$18,$1C
                db     $06,$4A,$2E,$18,$1C
                db     $06,$4C,$22,$18,$1C
                db     $06,$4C,$2E,$18,$1C
                db     $06,$4E,$22,$18,$1C
                db     $06,$4E,$2E,$18,$1C
                db     $06,$50,$22,$18,$1C
                db     $06,$50,$2E,$18,$1C
                db     $06,$52,$22,$18,$1C
                db     $06,$52,$2E,$18,$1C
                db     $06,$54,$22,$18,$1C
                db     $06,$54,$2E,$18,$1C
                db     $06,$56,$22,$18,$1C
                db     $06,$56,$2E,$18,$1C
                db     $06,$58,$22,$18,$1C
                db     $06,$58,$2E,$18,$1C
                db     $06,$56,$22,$18,$1C
                db     $06,$56,$2E,$18,$1C
                db     $06,$52,$22,$18,$1C
                db     $06,$52,$2E,$18,$1C
                db     $0C,$48,$26,$20,$18
                db     $0C,$00,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $06,$2C,$18,$0E,$08
                db     $06,$28,$18,$0E,$08
                db     $06,$26,$18,$0E,$08
                db     $06,$22,$18,$0E,$08
                db     $06,$20,$18,$0E,$08
                db     $06,$22,$18,$0E,$08
                db     $06,$26,$18,$0E,$08
                db     $06,$28,$18,$0E,$08
                db     $06,$2C,$18,$0E,$08
                db     $06,$28,$18,$0E,$08
                db     $06,$26,$18,$0E,$08
                db     $06,$22,$18,$0E,$08
                db     $06,$20,$18,$0E,$08
                db     $06,$22,$18,$0E,$08
                db     $06,$26,$18,$0E,$08
                db     $06,$28,$18,$0E,$08
                db     $06,$2C,$26,$20,$18
                db     $06,$2C,$00,$00,$18
                db     $06,$00,$26,$20,$18
                db     $06,$00,$00,$00,$18
                db     $06,$30,$26,$20,$18
                db     $06,$30,$00,$00,$18
                db     $06,$30,$26,$20,$18
                db     $06,$30,$00,$00,$18
                db     $06,$30,$26,$20,$18
                db     $06,$30,$00,$00,$18
                db     $06,$32,$26,$20,$18
                db     $06,$32,$00,$00,$18
                db     $06,$34,$26,$20,$18
                db     $06,$34,$00,$00,$18
                db     $06,$36,$26,$20,$18
                db     $06,$36,$00,$00,$18
                db     $06,$38,$26,$20,$18
                db     $06,$38,$00,$00,$18
                db     $06,$3A,$26,$20,$18
                db     $06,$3A,$00,$00,$18
                db     $06,$3C,$26,$20,$18
                db     $06,$3C,$00,$00,$18
                db     $06,$3E,$26,$20,$18
                db     $06,$3E,$00,$00,$18
                db     $06,$40,$26,$20,$18
                db     $06,$40,$00,$00,$18
                db     $06,$42,$26,$20,$18
                db     $06,$42,$00,$00,$18
                db     $06,$44,$26,$20,$18
                db     $06,$44,$00,$00,$18
                db     $06,$48,$26,$20,$18
                db     $06,$48,$00,$00,$18
                db     $0C,$40,$28,$22,$18
                db     $0C,$00,$00,$00,$00
                db     $06,$28,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$28,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$28,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$28,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$28,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$28,$00,$00,$00
                db     $06,$26,$00,$00,$00
                db     $06,$28,$16,$10,$0A
                db     $06,$26,$16,$10,$0A
                db     $06,$22,$16,$10,$0A
                db     $06,$1E,$16,$10,$0A
                db     $06,$1C,$16,$10,$0A
                db     $06,$1E,$16,$10,$0A
                db     $06,$22,$16,$10,$0A
                db     $06,$26,$16,$10,$0A
                db     $06,$28,$16,$10,$0A
                db     $06,$26,$16,$10,$0A
                db     $06,$22,$16,$10,$0A
                db     $06,$1E,$16,$10,$0A
                db     $06,$1C,$16,$10,$0A
                db     $06,$1E,$16,$10,$0A
                db     $06,$22,$16,$10,$0A
                db     $06,$26,$16,$10,$0A
                db     $06,$28,$22,$1C,$18
                db     $06,$28,$00,$1C,$18
                db     $06,$00,$22,$1C,$18
                db     $06,$00,$28,$1C,$18
                db     $06,$2C,$22,$1C,$18
                db     $06,$2C,$28,$1C,$18
                db     $06,$2C,$22,$1C,$18
                db     $06,$2C,$28,$1C,$18
                db     $06,$2C,$22,$1C,$18
                db     $06,$2C,$28,$1C,$18
                db     $06,$30,$22,$1C,$18
                db     $06,$30,$28,$1C,$18
                db     $06,$32,$22,$1C,$18
                db     $06,$32,$28,$1C,$18
                db     $06,$34,$22,$1C,$18
                db     $06,$34,$28,$1C,$18
                db     $06,$36,$28,$1C,$18
                db     $06,$36,$2E,$1C,$18
                db     $06,$38,$28,$1C,$18
                db     $06,$38,$2E,$1C,$18
                db     $06,$3A,$28,$1C,$18
                db     $06,$3A,$2E,$1C,$18
                db     $06,$3C,$28,$1C,$18
                db     $06,$3C,$2E,$1C,$18
                db     $06,$3E,$28,$1C,$18
                db     $06,$3E,$2E,$1C,$18
                db     $06,$3A,$28,$1C,$18
                db     $06,$3A,$2E,$18,$1C
                db     $0C,$38,$26,$18,$00
                db     $0C,$00,$00,$00,$00
                db     $06,$18,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$18,$00,$00,$00
                db     $06,$2C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$00,$00,$00
                db     $18,$00,$18,$00,$00
                db     $18,$52,$32,$00,$00
                db     $18,$00,$18,$00,$00
                db     $18,$56,$32,$00,$00
                db     $18,$20,$18,$00,$00
                db     $18,$2C,$48,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$52,$32,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$56,$32,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$48,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$52,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$56,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$48,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$2C,$00,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$52,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$32,$56,$00,$00
                db     $18,$18,$00,$00,$00
                db     $18,$30,$48,$00,$00
                db     $18,$30,$18,$00,$00
                db     $18,$2E,$18,$00,$00
                db     $18,$30,$2C,$18,$00
                db     $18,$2E,$2C,$18,$00
                db     $18,$30,$2A,$18,$00
                db     $18,$2E,$2A,$18,$00
                db     $18,$30,$28,$18,$00
                db     $18,$2E,$28,$18,$00
                db     $18,$30,$2C,$18,$26
                db     $18,$2E,$2C,$18,$26
                db     $18,$30,$2A,$24,$18
                db     $18,$2E,$2A,$24,$18
                db     $18,$30,$28,$22,$18
                db     $18,$2E,$28,$22,$18
                db     $0C,$30,$26,$20,$18
                db     $0C,$30,$00,$00,$18
                db     $18,$2E,$00,$00,$18
                db     $18,$30,$16,$00,$00
                db     $18,$2E,$16,$00,$00
                db     $18,$30,$14,$00,$00
                db     $18,$2E,$14,$00,$00
                db     $18,$30,$18,$12,$00
                db     $18,$2E,$18,$12,$00
                db     $18,$30,$12,$14,$00
                db     $18,$2E,$10,$16,$00
                db     $18,$30,$0E,$14,$00
                db     $18,$2E,$0E,$14,$00
                db     $18,$30,$0C,$12,$00
                db     $18,$2E,$0C,$12,$00
                db     $18,$30,$0A,$10,$00
                db     $18,$2E,$0A,$10,$00
                db     $0C,$30,$0E,$08,$00
                db     $48,$00,$00,$00,$00
                db     $06,$56,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $0C,$60,$00,$00,$00
                db     $54,$00,$00,$00,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $0C,$38,$30,$26,$18
                db     $48,$00,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$56,$00,$00,$00
                db     $0C,$56,$00,$00,$00
                db     $54,$00,$00,$00,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $18,$3A,$26,$18,$00
                db     $18,$32,$2C,$18,$00
                db     $0C,$38,$30,$26,$18
                db     $48,$00,$00,$00,$00
                db     $05,$3E,$00,$00,$00
                db     $05,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $48,$00,$00,$00,$00
                db     $05,$3E,$00,$00,$00
                db     $05,$56,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $18,$56,$00,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $05,$3E,$00,$00,$00
                db     $05,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $18,$48,$00,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $05,$3E,$00,$00,$00
                db     $05,$56,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $18,$56,$00,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $05,$3E,$00,$00,$00
                db     $05,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $18,$48,$00,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $05,$3E,$00,$00,$00
                db     $05,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $3C,$48,$00,$00,$00
                db     $24,$00,$00,$00,$00
                db     $20,$40,$30,$1E,$10
                db     $08,$44,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$40,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $20,$40,$30,$1E,$10
                db     $08,$44,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$40,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $20,$40,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$52,$30,$1E,$10
                db     $08,$4E,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $44,$44,$1E,$26,$2C
                db     $02,$42,$1E,$26,$2C
                db     $02,$44,$1E,$26,$2C
                db     $18,$4E,$1E,$26,$2C
                db     $30,$00,$00,$00,$00
                db     $20,$40,$30,$28,$1E
                db     $08,$44,$30,$28,$1E
                db     $08,$36,$30,$28,$1E
                db     $08,$40,$30,$28,$1E
                db     $08,$48,$30,$28,$1E
                db     $08,$36,$30,$28,$1E
                db     $20,$40,$30,$28,$1E
                db     $08,$44,$30,$28,$1E
                db     $08,$36,$30,$28,$1E
                db     $08,$40,$30,$28,$1E
                db     $08,$48,$30,$28,$1E
                db     $08,$36,$30,$28,$1E
                db     $20,$40,$30,$28,$1E
                db     $08,$48,$30,$28,$1E
                db     $08,$52,$30,$28,$1E
                db     $08,$4E,$30,$28,$1E
                db     $08,$36,$30,$28,$1E
                db     $08,$48,$30,$28,$1E
                db     $44,$44,$2C,$26,$1E
                db     $02,$42,$2C,$26,$1E
                db     $02,$44,$2C,$26,$1E
                db     $18,$4E,$2C,$26,$1E
                db     $30,$00,$00,$00,$00
                db     $18,$44,$3A,$32,$2C
                db     $08,$44,$00,$32,$2C
                db     $08,$48,$00,$32,$2C
                db     $08,$3A,$00,$32,$2C
                db     $08,$44,$00,$32,$2C
                db     $08,$48,$00,$32,$2C
                db     $08,$3A,$00,$32,$2C
                db     $20,$44,$32,$26,$1E
                db     $08,$48,$32,$26,$1E
                db     $08,$3A,$32,$26,$1E
                db     $08,$44,$32,$26,$1E
                db     $08,$48,$32,$26,$1E
                db     $08,$3A,$32,$26,$1E
                db     $20,$40,$30,$28,$00
                db     $08,$44,$30,$28,$00
                db     $08,$48,$30,$28,$00
                db     $08,$4C,$34,$2E,$28
                db     $08,$4E,$34,$2E,$28
                db     $08,$52,$34,$2E,$28
                db     $44,$4E,$40,$30,$28
                db     $02,$4C,$40,$30,$28
                db     $02,$4E,$40,$30,$28
                db     $18,$58,$40,$30,$28
                db     $30,$00,$00,$00,$00
                db     $20,$5C,$3A,$32,$2C
                db     $08,$60,$3A,$32,$2C
                db     $08,$52,$3A,$32,$2C
                db     $08,$5C,$3A,$32,$2C
                db     $08,$60,$3A,$32,$2C
                db     $08,$52,$3A,$32,$2C
                db     $20,$5C,$32,$26,$1E
                db     $08,$60,$32,$26,$1E
                db     $08,$52,$32,$26,$1E
                db     $08,$5C,$32,$26,$1E
                db     $08,$60,$32,$26,$1E
                db     $08,$52,$32,$26,$1E
                db     $20,$58,$30,$28,$00
                db     $08,$5C,$30,$28,$00
                db     $08,$60,$30,$28,$00
                db     $08,$64,$34,$2E,$28
                db     $08,$66,$34,$2E,$28
                db     $08,$6A,$34,$2E,$28
                db     $44,$66,$40,$30,$28
                db     $02,$64,$40,$30,$28
                db     $02,$66,$40,$30,$28
                db     $18,$70,$40,$30,$28
                db     $30,$00,$00,$00,$00
                db     $08,$36,$30,$1E,$10
                db     $08,$3A,$30,$1E,$10
                db     $08,$3E,$30,$1E,$10
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$4A,$32,$22,$10
                db     $08,$3A,$32,$22,$10
                db     $08,$4A,$32,$22,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$44,$26,$1E,$10
                db     $08,$32,$26,$1E,$10
                db     $08,$44,$26,$1E,$10
                db     $08,$40,$28,$1E,$10
                db     $08,$30,$28,$1E,$10
                db     $08,$40,$28,$1E,$10
                db     $08,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $08,$44,$2C,$26,$1E
                db     $10,$4A,$2C,$26,$1E
                db     $08,$48,$30,$28,$1E
                db     $10,$40,$30,$28,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $08,$44,$2C,$26,$1E
                db     $10,$4A,$2C,$26,$1E
                db     $08,$48,$30,$28,$1E
                db     $10,$40,$30,$28,$1E
                db     $10,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $10,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $10,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $18,$4E,$2C,$26,$1E
                db     $30,$4E,$00,$00,$00
                db     $08,$36,$30,$1E,$10
                db     $08,$3A,$30,$1E,$10
                db     $08,$3E,$30,$1E,$10
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$4A,$32,$22,$10
                db     $08,$3A,$32,$22,$10
                db     $08,$4A,$32,$22,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$44,$26,$1E,$10
                db     $08,$32,$26,$1E,$10
                db     $08,$44,$26,$1E,$10
                db     $08,$40,$28,$1E,$10
                db     $08,$30,$28,$1E,$10
                db     $08,$40,$28,$1E,$10
                db     $08,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $08,$44,$2C,$26,$1E
                db     $10,$4A,$2C,$26,$1E
                db     $08,$48,$30,$28,$1E
                db     $10,$40,$30,$28,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $08,$36,$2C,$26,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $10,$44,$2C,$26,$1E
                db     $08,$3E,$2C,$26,$1E
                db     $10,$40,$28,$22,$18
                db     $08,$3A,$28,$22,$18
                db     $08,$38,$20,$18,$26
                db     $10,$48,$00,$00,$00
                db     $08,$38,$20,$18,$26
                db     $10,$48,$00,$00,$00
                db     $08,$38,$20,$18,$26
                db     $10,$48,$00,$00,$00
                db     $18,$48,$20,$18,$26
                db     $30,$48,$00,$00,$00
                db     $20,$48,$00,$00,$00
                db     $08,$52,$00,$00,$00
                db     $08,$4E,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$4A,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $18,$40,$30,$1E,$10
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$36,$00,$00,$00
                db     $08,$40,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $18,$40,$30,$1E,$10
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$4A,$32,$22,$10
                db     $08,$4E,$32,$22,$10
                db     $08,$52,$32,$22,$10
                db     $18,$4E,$30,$1E,$10
                db     $08,$4E,$00,$00,$00
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$46,$34,$2E,$22
                db     $08,$40,$34,$2E,$22
                db     $08,$44,$34,$2E,$22
                db     $20,$48,$36,$30,$1E
                db     $08,$44,$36,$30,$1E
                db     $08,$40,$36,$30,$1E
                db     $08,$44,$32,$26,$1E
                db     $08,$36,$32,$26,$1E
                db     $08,$4E,$32,$26,$1E
                db     $18,$40,$30,$1E,$10
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$36,$00,$00,$00
                db     $08,$40,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $18,$40,$30,$1E,$10
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$36,$00,$00,$00
                db     $08,$4A,$32,$22,$10
                db     $08,$4E,$32,$22,$10
                db     $08,$52,$32,$22,$10
                db     $18,$4E,$30,$1E,$10
                db     $08,$4E,$00,$00,$00
                db     $08,$40,$00,$00,$00
                db     $08,$44,$00,$00,$00
                db     $08,$46,$34,$2E,$22
                db     $08,$40,$34,$2E,$22
                db     $08,$44,$34,$2E,$22
                db     $20,$48,$36,$30,$1E
                db     $08,$44,$36,$30,$1E
                db     $08,$40,$36,$30,$1E
                db     $08,$44,$32,$26,$1E
                db     $08,$36,$32,$26,$1E
                db     $08,$4E,$32,$26,$1E
                db     $08,$40,$30,$28,$00
                db     $08,$58,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$00,$00,$00
                db     $08,$40,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$4E,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$40,$00,$00,$00
                db     $20,$4E,$44,$26,$1E
                db     $08,$4E,$48,$26,$1E
                db     $08,$4E,$3A,$26,$1E
                db     $08,$44,$4E,$26,$1E
                db     $08,$48,$4E,$26,$1E
                db     $08,$36,$4E,$26,$1E
                db     $08,$40,$30,$28,$00
                db     $08,$58,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$00,$00,$00
                db     $08,$40,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$4E,$00,$00,$00
                db     $08,$48,$00,$00,$00
                db     $08,$40,$00,$00,$00
                db     $20,$4E,$44,$26,$1E
                db     $08,$4E,$48,$26,$1E
                db     $08,$4E,$3A,$26,$1E
                db     $08,$44,$4E,$26,$1E
                db     $08,$48,$4E,$26,$1E
                db     $08,$36,$4E,$26,$1E
                db     $20,$40,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$30,$28,$00
                db     $08,$40,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $20,$40,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$30,$28,$00
                db     $08,$40,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $20,$40,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$30,$28,$00
                db     $08,$40,$30,$1E,$10
                db     $08,$36,$30,$1E,$10
                db     $08,$48,$30,$1E,$10
                db     $20,$40,$30,$28,$00
                db     $08,$58,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$30,$1E,$10
                db     $08,$40,$30,$1E,$10
                db     $08,$4E,$30,$1E,$10
                db     $08,$48,$30,$28,$00
                db     $08,$40,$30,$28,$00
                db     $08,$4E,$30,$28,$00
                db     $08,$48,$30,$28,$1E
                db     $08,$40,$30,$28,$1E
                db     $08,$4E,$30,$28,$1E
                db     $08,$48,$28,$1E,$18
                db     $08,$40,$28,$1E,$18
                db     $08,$4E,$28,$1E,$18
                db     $0C,$48,$28,$1E,$10
                db     $0C,$48,$00,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $18,$48,$00,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $0C,$42,$00,$00,$00
                db     $0C,$3A,$00,$00,$00
                db     $0C,$42,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $0C,$42,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0C,$48,$3A,$00,$00
                db     $0C,$42,$3A,$00,$00
                db     $0C,$3A,$00,$00,$00
                db     $05,$42,$3A,$00,$00
                db     $02,$42,$00,$00,$00
                db     $05,$42,$3A,$00,$00
                db     $0C,$48,$3A,$00,$00
                db     $0C,$42,$3A,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $05,$52,$3A,$42,$00
                db     $02,$52,$00,$00,$00
                db     $05,$52,$3A,$42,$00
                db     $0C,$48,$3A,$42,$00
                db     $0C,$48,$3E,$30,$00
                db     $0C,$00,$3A,$2A,$00
                db     $05,$48,$3E,$30,$00
                db     $02,$00,$3E,$30,$00
                db     $05,$48,$3E,$30,$00
                db     $0C,$48,$3A,$42,$00
                db     $0C,$48,$42,$48,$00
                db     $0C,$00,$3E,$44,$00
                db     $05,$48,$3A,$42,$00
                db     $02,$00,$3A,$42,$00
                db     $05,$48,$3A,$42,$00
                db     $0C,$48,$3E,$30,$00
                db     $0C,$48,$3A,$42,$00
                db     $0C,$00,$3E,$30,$00
                db     $05,$48,$3E,$44,$00
                db     $02,$00,$3E,$44,$00
                db     $05,$48,$3E,$44,$00
                db     $0C,$48,$3A,$42,$00
                db     $0C,$48,$3E,$30,$00
                db     $0C,$00,$3A,$2A,$00
                db     $05,$48,$3A,$42,$00
                db     $02,$00,$3A,$42,$00
                db     $05,$48,$3A,$42,$00
                db     $0C,$48,$3E,$30,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $04,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$3E,$38,$30
                db     $30,$48,$3E,$38,$30
                db     $30,$48,$3E,$38,$30
                db     $0C,$48,$3E,$38,$30
                db     $24,$00,$00,$00,$00
                db     $24,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3E,$2A,$22,$00
                db     $0C,$42,$2A,$22,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $05,$42,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$2A,$22,$00
                db     $0C,$3E,$26,$20,$18
                db     $0C,$38,$26,$20,$18
                db     $0C,$30,$26,$20,$18
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3E,$2A,$22,$00
                db     $0C,$42,$2A,$22,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $0B,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$44,$26,$20,$18
                db     $06,$42,$26,$20,$18
                db     $06,$3E,$26,$20,$18
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$42,$30,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$48,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$30,$48,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$30,$48,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0C,$30,$48,$2A,$22
                db     $05,$30,$48,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $05,$30,$48,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $0C,$56,$30,$2A,$22
                db     $0C,$5A,$30,$2A,$22
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$30,$2A,$22
                db     $05,$00,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $05,$00,$30,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $05,$5A,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $05,$5A,$30,$2A,$22
                db     $0C,$56,$26,$1E,$18
                db     $0C,$50,$26,$1E,$18
                db     $0C,$48,$26,$1E,$18
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$30,$2A,$22
                db     $05,$00,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $05,$00,$30,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $0C,$56,$30,$2A,$22
                db     $0C,$5A,$30,$2A,$22
                db     $06,$52,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $0B,$60,$48,$26,$18
                db     $01,$00,$00,$00,$00
                db     $05,$60,$48,$26,$18
                db     $01,$00,$00,$00,$00
                db     $05,$60,$48,$26,$18
                db     $01,$00,$00,$00,$00
                db     $06,$60,$48,$26,$18
                db     $06,$5C,$00,$26,$18
                db     $06,$5A,$00,$26,$18
                db     $06,$56,$00,$26,$18
                db     $0C,$52,$00,$2A,$22
                db     $0C,$5A,$48,$2A,$22
                db     $0C,$52,$42,$2A,$22
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$2A,$12
                db     $0C,$50,$56,$00,$00
                db     $0C,$52,$4C,$2A,$1C
                db     $0C,$50,$1A,$26,$1C
                db     $0C,$2A,$1C,$22,$1C
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$60,$56,$38,$26
                db     $0C,$5E,$52,$34,$26
                db     $0C,$60,$50,$30,$18
                db     $0C,$5E,$52,$28,$18
                db     $0C,$60,$50,$26,$18
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$2A,$12
                db     $0C,$50,$56,$00,$00
                db     $0C,$52,$4C,$2A,$1C
                db     $0C,$50,$4A,$26,$1C
                db     $0C,$4C,$2A,$22,$1C
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$26
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$60,$56,$38,$26
                db     $0C,$5E,$52,$34,$26
                db     $0C,$60,$50,$30,$18
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $04,$3E,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$3E,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$3E,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$3E,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$3E,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3E,$30,$00,$00
                db     $0C,$3A,$42,$00,$00
                db     $0C,$3E,$44,$00,$00
                db     $0C,$3E,$30,$18,$00
                db     $0C,$3E,$30,$1C,$00
                db     $0C,$3E,$44,$00,$00
                db     $0C,$42,$3A,$22,$00
                db     $0C,$3A,$26,$00,$00
                db     $0C,$3A,$2A,$00,$00
                db     $0C,$42,$3A,$2C,$00
                db     $0C,$3E,$38,$30,$00
                db     $0C,$3E,$38,$18,$00
                db     $0C,$3E,$38,$30,$00
                db     $04,$56,$48,$3E,$30
                db     $02,$00,$00,$00,$00
                db     $04,$56,$48,$3E,$30
                db     $02,$00,$00,$00,$00
                db     $0A,$56,$48,$3E,$30
                db     $02,$00,$00,$00,$00
                db     $04,$56,$48,$3E,$30
                db     $02,$00,$00,$00,$00
                db     $04,$56,$48,$3E,$30
                db     $02,$00,$00,$00,$00
                db     $0C,$56,$48,$3E,$30
                db     $0C,$5A,$52,$42,$3A
                db     $0C,$5C,$56,$44,$3E
                db     $0C,$56,$48,$18,$00
                db     $0C,$56,$48,$1C,$00
                db     $0C,$5C,$56,$20,$00
                db     $0C,$5A,$52,$22,$00
                db     $0C,$52,$42,$26,$00
                db     $0C,$52,$42,$2A,$00
                db     $0C,$5A,$52,$2C,$00
                db     $0C,$56,$50,$30,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3E,$2A,$22,$00
                db     $0C,$42,$2A,$22,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$30,$2A,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $05,$42,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$2A,$22,$00
                db     $0C,$3E,$26,$20,$18
                db     $0C,$38,$26,$20,$18
                db     $0C,$30,$26,$20,$18
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3E,$2A,$22,$00
                db     $0C,$42,$2A,$22,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $0B,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$44,$26,$20,$18
                db     $06,$42,$26,$20,$18
                db     $06,$3E,$26,$20,$18
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$42,$30,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $04,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$60,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0C,$52,$30,$2A,$22
                db     $0C,$56,$30,$2A,$22
                db     $0C,$5A,$30,$2A,$22
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$30,$2A,$22
                db     $05,$00,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $06,$00,$30,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $05,$5A,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $05,$5A,$30,$2A,$22
                db     $0C,$56,$26,$20,$18
                db     $0C,$50,$26,$20,$18
                db     $0C,$48,$26,$20,$18
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$30,$2A,$22
                db     $05,$00,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $06,$00,$30,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $0C,$56,$30,$2A,$22
                db     $0C,$5A,$30,$2A,$22
                db     $06,$52,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $0B,$60,$48,$20,$18
                db     $01,$60,$00,$00,$00
                db     $05,$60,$48,$20,$18
                db     $01,$60,$00,$00,$00
                db     $05,$60,$48,$20,$18
                db     $01,$60,$00,$00,$00
                db     $06,$60,$48,$20,$18
                db     $06,$5C,$48,$20,$18
                db     $06,$5A,$48,$20,$18
                db     $06,$56,$48,$20,$18
                db     $0C,$52,$2A,$22,$00
                db     $0B,$5A,$48,$2A,$22
                db     $0B,$52,$42,$2A,$22
                db     $02,$52,$00,$00,$00
                db     $02,$56,$00,$00,$00
                db     $02,$5A,$00,$00,$00
                db     $02,$5C,$00,$00,$00
                db     $02,$60,$00,$00,$00
                db     $02,$64,$00,$00,$00
                db     $02,$68,$00,$00,$00
                db     $0C,$6A,$00,$2A,$22
                db     $04,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $06,$72,$5A,$32,$00
                db     $06,$72,$5A,$22,$00
                db     $06,$72,$5A,$32,$00
                db     $06,$72,$5A,$22,$00
                db     $06,$72,$5A,$32,$00
                db     $06,$72,$5A,$22,$00
                db     $06,$6E,$00,$56,$32
                db     $06,$6E,$00,$56,$22
                db     $06,$6A,$64,$52,$34
                db     $06,$6A,$64,$52,$22
                db     $06,$68,$50,$32,$00
                db     $06,$68,$50,$22,$00
                db     $06,$6A,$52,$32,$00
                db     $06,$6A,$52,$22,$00
                db     $06,$64,$4C,$2C,$00
                db     $06,$64,$4C,$22,$00
                db     $06,$60,$2A,$22,$00
                db     $06,$64,$2A,$22,$00
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$60,$30,$2A,$22
                db     $06,$64,$30,$2A,$22
                db     $06,$68,$00,$00,$00
                db     $06,$6A,$00,$00,$00
                db     $06,$5C,$2C,$26,$20
                db     $06,$60,$2C,$26,$20
                db     $06,$5C,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $06,$5C,$2C,$26,$20
                db     $06,$60,$2C,$26,$20
                db     $06,$64,$00,$00,$00
                db     $06,$68,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$5A,$00,$00,$00
                db     $06,$5C,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$56,$26,$1C,$14
                db     $06,$5A,$26,$1C,$14
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$26,$1C,$14
                db     $06,$5A,$26,$1C,$14
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$5A,$26,$20,$18
                db     $06,$56,$00,$00,$00
                db     $06,$50,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$68,$00,$00,$00
                db     $0C,$6A,$00,$2A,$22
                db     $04,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0B,$6A,$52,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0B,$6A,$52,$2A,$22
                db     $06,$72,$00,$5A,$32
                db     $06,$72,$00,$5A,$22
                db     $06,$72,$00,$5A,$32
                db     $06,$72,$00,$5A,$22
                db     $06,$72,$00,$5A,$32
                db     $06,$72,$00,$5A,$22
                db     $06,$6E,$00,$56,$32
                db     $06,$6E,$00,$56,$22
                db     $06,$6A,$64,$52,$34
                db     $06,$6A,$64,$52,$22
                db     $06,$68,$50,$00,$32
                db     $06,$68,$50,$00,$22
                db     $06,$6A,$52,$00,$34
                db     $06,$6A,$52,$00,$22
                db     $06,$64,$4C,$00,$2C
                db     $06,$64,$4C,$00,$22
                db     $06,$60,$2A,$22,$00
                db     $06,$64,$2A,$22,$00
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$60,$30,$2A,$22
                db     $06,$64,$30,$2A,$22
                db     $06,$68,$00,$00,$00
                db     $06,$6A,$00,$00,$00
                db     $06,$5C,$2C,$26,$20
                db     $06,$60,$2C,$26,$20
                db     $06,$5C,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $06,$5C,$2C,$26,$20
                db     $06,$60,$2C,$26,$20
                db     $06,$64,$00,$00,$00
                db     $06,$68,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$5A,$00,$00,$00
                db     $06,$5C,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$56,$26,$14,$1C
                db     $06,$5A,$26,$14,$1C
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$18,$26,$20
                db     $06,$5C,$18,$26,$20
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$00,$00,$00
                db     $0C,$52,$2A,$22,$00
                db     $0C,$00,$00,$00,$00
                db     $18,$4C,$00,$00,$00
                db     $06,$4C,$34,$22,$1C
                db     $06,$4A,$34,$22,$1C
                db     $06,$4C,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$50,$34,$2C,$26
                db     $06,$4C,$34,$2C,$26
                db     $06,$48,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$44,$34,$22,$1C
                db     $06,$42,$34,$22,$1C
                db     $06,$3A,$00,$00,$00
                db     $04,$34,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$34,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$34,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$34,$2A,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$34,$2A,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $06,$3A,$2A,$1A,$12
                db     $06,$38,$2A,$1A,$12
                db     $06,$32,$00,$00,$00
                db     $06,$2A,$00,$00,$00
                db     $05,$2A,$20,$18,$12
                db     $02,$00,$00,$00,$00
                db     $05,$2A,$20,$18,$12
                db     $05,$2A,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$2A,$00,$00,$00
                db     $06,$2A,$22,$1C,$00
                db     $06,$34,$22,$1C,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $04,$4C,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$4C,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$4C,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$4C,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $06,$4C,$34,$22,$1C
                db     $06,$4A,$34,$22,$1C
                db     $06,$4C,$00,$00,$00
                db     $06,$4A,$00,$00,$00
                db     $06,$50,$34,$22,$1C
                db     $06,$4C,$34,$22,$1C
                db     $06,$48,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$44,$34,$22,$1C
                db     $06,$42,$34,$22,$1C
                db     $05,$3A,$00,$00,$00
                db     $05,$34,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$34,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$34,$00,$00,$00
                db     $05,$34,$2A,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $05,$34,$2A,$22,$1C
                db     $06,$3A,$2A,$18,$12
                db     $06,$38,$2A,$18,$12
                db     $05,$32,$00,$00,$00
                db     $05,$2A,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$2A,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$2A,$00,$00,$00
                db     $05,$2A,$20,$18,$12
                db     $02,$00,$00,$00,$00
                db     $05,$2A,$20,$18,$12
                db     $06,$2A,$22,$1C,$00
                db     $06,$34,$20,$1C,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$52,$00,$00,$00
                db     $06,$50,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$50,$38,$32,$2A
                db     $06,$4A,$38,$32,$2A
                db     $05,$42,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$00,$00,$00
                db     $05,$5A,$00,$00,$00
                db     $06,$52,$3A,$34,$2A
                db     $06,$4C,$3A,$34,$2A
                db     $06,$50,$38,$32,$2A
                db     $06,$4A,$38,$32,$2A
                db     $05,$42,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$00,$00,$00
                db     $05,$5A,$00,$00,$00
                db     $06,$52,$3A,$34,$2A
                db     $06,$4C,$3A,$34,$2A
                db     $06,$50,$38,$30,$2A
                db     $06,$48,$38,$30,$2A
                db     $05,$42,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$42,$00,$00,$00
                db     $06,$46,$34,$26,$20
                db     $06,$3E,$34,$26,$20
                db     $05,$36,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$36,$00,$00,$00
                db     $06,$42,$30,$2A,$00
                db     $06,$3E,$30,$2A,$00
                db     $06,$42,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$40,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$30,$2A,$22,$00
                db     $06,$2C,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$3A,$22,$12,$00
                db     $06,$30,$22,$0A,$00
                db     $06,$3E,$26,$18,$00
                db     $06,$30,$26,$18,$00
                db     $06,$42,$2A,$22,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$38,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$2A,$22,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$44,$2C,$26,$00
                db     $06,$3E,$2C,$26,$00
                db     $06,$48,$30,$2A,$22
                db     $06,$5A,$2A,$2A,$22
                db     $06,$56,$00,$00,$00
                db     $06,$52,$00,$00,$00
                db     $06,$50,$34,$22,$2E
                db     $06,$56,$34,$22,$2E
                db     $06,$52,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$48,$30,$2A,$22
                db     $06,$4C,$30,$2A,$22
                db     $06,$48,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$50,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$44,$20,$26,$18
                db     $06,$48,$20,$26,$18
                db     $06,$44,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4C,$20,$26,$18
                db     $06,$50,$20,$26,$18
                db     $06,$52,$00,$00,$00
                db     $06,$56,$00,$00,$00
                db     $06,$42,$30,$2A,$22
                db     $06,$44,$30,$2A,$22
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$48,$2A,$30,$22
                db     $06,$4C,$2A,$30,$22
                db     $06,$50,$00,$00,$00
                db     $06,$52,$00,$00,$00
                db     $06,$3E,$14,$1C,$22
                db     $06,$42,$14,$1C,$22
                db     $06,$3E,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$3E,$14,$1C,$22
                db     $06,$42,$14,$1C,$22
                db     $06,$3E,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$3E,$26,$20,$18
                db     $06,$42,$26,$20,$18
                db     $06,$3E,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$32,$00,$00,$00
                db     $06,$34,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$30,$2A,$22,$00
                db     $06,$2E,$00,$00,$00
                db     $06,$30,$00,$00,$00
                db     $06,$3A,$22,$12,$00
                db     $06,$30,$22,$12,$00
                db     $06,$3E,$26,$18,$00
                db     $06,$30,$26,$18,$00
                db     $06,$42,$2A,$22,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$38,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$2A,$22,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$44,$2C,$26,$00
                db     $06,$3E,$2C,$26,$00
                db     $06,$48,$30,$2A,$22
                db     $06,$5A,$2A,$2A,$22
                db     $06,$56,$00,$00,$00
                db     $06,$52,$00,$00,$00
                db     $06,$50,$34,$22,$2E
                db     $06,$56,$34,$22,$2E
                db     $06,$52,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$48,$30,$2A,$22
                db     $06,$4C,$30,$2A,$22
                db     $06,$48,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$50,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$44,$20,$26,$18
                db     $06,$48,$20,$26,$18
                db     $06,$44,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4C,$20,$26,$18
                db     $06,$50,$20,$26,$18
                db     $06,$52,$00,$00,$00
                db     $06,$56,$00,$00,$00
                db     $06,$42,$30,$2A,$22
                db     $06,$44,$30,$2A,$22
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$48,$2A,$30,$22
                db     $06,$4C,$2A,$30,$22
                db     $06,$50,$00,$00,$00
                db     $06,$52,$00,$00,$00
                db     $06,$3E,$26,$22,$14
                db     $06,$44,$26,$22,$14
                db     $06,$4A,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$50,$2C,$20,$18
                db     $06,$4C,$2C,$20,$18
                db     $06,$44,$00,$00,$00
                db     $06,$3E,$00,$00,$00
                db     $05,$3A,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $05,$3A,$2A,$22,$00
                db     $06,$3E,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$48,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$50,$00,$00,$00
                db     $0A,$52,$3A,$22,$2A
                db     $02,$00,$00,$00,$00
                db     $04,$52,$3A,$22,$2A
                db     $02,$00,$00,$00,$00
                db     $04,$52,$3A,$22,$2A
                db     $02,$00,$00,$00,$00
                db     $0A,$52,$3A,$22,$2A
                db     $02,$00,$00,$00,$00
                db     $0A,$52,$3A,$22,$2A
                db     $02,$00,$00,$00,$00
                db     $06,$5A,$4A,$32,$00
                db     $06,$5A,$4A,$22,$00
                db     $06,$5A,$4A,$32,$00
                db     $06,$5A,$4A,$22,$00
                db     $06,$5A,$4A,$32,$00
                db     $06,$5A,$4A,$22,$00
                db     $06,$56,$3E,$32,$00
                db     $06,$50,$3E,$22,$00
                db     $06,$52,$4C,$3A,$34
                db     $06,$52,$4C,$3A,$22
                db     $06,$50,$38,$00,$32
                db     $06,$50,$38,$00,$22
                db     $06,$52,$3A,$00,$34
                db     $06,$52,$3A,$00,$22
                db     $06,$4C,$36,$2C,$00
                db     $06,$4C,$36,$22,$00
                db     $06,$60,$2A,$22,$00
                db     $06,$64,$2A,$22,$00
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$60,$30,$2A,$22
                db     $06,$64,$30,$2A,$22
                db     $06,$68,$00,$00,$00
                db     $06,$6A,$00,$00,$00
                db     $06,$5C,$30,$2C,$20
                db     $06,$60,$30,$2C,$20
                db     $06,$5C,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $06,$5C,$30,$2C,$20
                db     $06,$60,$30,$2C,$20
                db     $06,$64,$00,$00,$00
                db     $06,$68,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$5A,$00,$00,$00
                db     $06,$5C,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$56,$26,$1C,$14
                db     $06,$5A,$26,$1C,$14
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$26,$1C,$14
                db     $06,$5A,$26,$1C,$14
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$5A,$26,$20,$18
                db     $06,$56,$00,$00,$00
                db     $06,$50,$00,$00,$00
                db     $05,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$00,$00,$00
                db     $06,$4C,$00,$00,$00
                db     $06,$50,$00,$00,$00
                db     $0A,$52,$3A,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$52,$3A,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $04,$52,$3A,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0B,$52,$3A,$2A,$22
                db     $02,$00,$00,$00,$00
                db     $0B,$52,$3A,$2A,$22
                db     $06,$5A,$4A,$32,$00
                db     $06,$5A,$4A,$22,$00
                db     $06,$5A,$4A,$32,$00
                db     $06,$5A,$4A,$22,$00
                db     $06,$5A,$4A,$32,$00
                db     $06,$5A,$4A,$22,$00
                db     $06,$56,$3E,$32,$00
                db     $06,$56,$3E,$22,$00
                db     $06,$52,$4C,$3A,$34
                db     $06,$52,$4C,$3A,$22
                db     $06,$50,$38,$00,$32
                db     $06,$50,$38,$00,$22
                db     $06,$52,$3A,$00,$34
                db     $06,$52,$3A,$00,$22
                db     $06,$4C,$34,$2C,$00
                db     $06,$4C,$34,$22,$00
                db     $06,$60,$2A,$22,$00
                db     $06,$64,$2A,$22,$00
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$60,$30,$2A,$22
                db     $06,$64,$30,$2A,$22
                db     $06,$68,$00,$00,$00
                db     $06,$6A,$00,$00,$00
                db     $06,$5C,$30,$2C,$20
                db     $06,$60,$30,$2C,$20
                db     $06,$5C,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $06,$5C,$30,$2C,$20
                db     $06,$60,$30,$2C,$20
                db     $06,$64,$00,$00,$00
                db     $06,$68,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$5A,$00,$00,$00
                db     $06,$5C,$00,$00,$00
                db     $06,$5A,$30,$2A,$22
                db     $06,$5C,$30,$2A,$22
                db     $06,$60,$00,$00,$00
                db     $06,$64,$00,$00,$00
                db     $06,$56,$26,$1C,$14
                db     $06,$5A,$26,$1C,$14
                db     $06,$56,$00,$00,$00
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$26,$20,$18
                db     $06,$5C,$26,$20,$18
                db     $06,$5A,$00,$00,$00
                db     $06,$56,$00,$00,$00
                db     $0C,$52,$00,$2A,$22
                db     $0C,$48,$30,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $04,$5A,$52,$1C,$22
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$1C,$22
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$22,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$22,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$2A,$12
                db     $0C,$56,$50,$00,$00
                db     $0C,$52,$4C,$2A,$1C
                db     $0C,$50,$4A,$26,$1C
                db     $0C,$4C,$2A,$22,$1C
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$60,$56,$38,$26
                db     $0C,$5E,$52,$34,$26
                db     $0C,$60,$50,$30,$18
                db     $0C,$5E,$52,$28,$18
                db     $0C,$60,$50,$26,$18
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$22,$1C
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$2A,$12
                db     $0C,$56,$50,$00,$00
                db     $0C,$52,$4C,$2A,$1C
                db     $0C,$50,$4A,$26,$1C
                db     $0C,$4C,$2A,$22,$1C
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0A,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$4C,$00,$00
                db     $0C,$5A,$52,$1C,$22
                db     $0C,$64,$4C,$00,$00
                db     $0C,$60,$50,$30,$18
                db     $0C,$5E,$52,$28,$18
                db     $0C,$60,$50,$26,$18
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$1C
                db     $02,$00,$00,$00,$00
                db     $06,$60,$50,$30,$18
                db     $06,$5E,$50,$30,$18
                db     $06,$64,$00,$00,$00
                db     $06,$5E,$00,$00,$00
                db     $06,$60,$00,$00,$00
                db     $06,$5E,$00,$00,$00
                db     $06,$64,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$60,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$64,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$60,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$64,$42,$3A,$00
                db     $06,$60,$42,$3A,$00
                db     $06,$60,$44,$3E,$00
                db     $06,$5E,$44,$3E,$00
                db     $06,$64,$30,$3E,$00
                db     $06,$5E,$30,$3E,$00
                db     $06,$60,$30,$3E,$00
                db     $06,$5E,$30,$3E,$00
                db     $06,$64,$44,$3E,$00
                db     $06,$5E,$44,$3E,$00
                db     $06,$60,$42,$3A,$00
                db     $06,$5E,$42,$3A,$00
                db     $06,$64,$2A,$3A,$00
                db     $06,$5E,$2A,$3A,$00
                db     $06,$60,$2A,$3A,$00
                db     $06,$5E,$2A,$3A,$00
                db     $06,$64,$42,$3A,$00
                db     $06,$5E,$42,$3A,$00
                db     $06,$60,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$64,$42,$00,$00
                db     $06,$5E,$42,$00,$00
                db     $06,$60,$30,$00,$00
                db     $06,$5E,$30,$00,$00
                db     $06,$64,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$60,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$64,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$60,$3E,$30,$00
                db     $06,$5E,$3E,$30,$00
                db     $06,$64,$42,$3A,$00
                db     $06,$60,$42,$3A,$00
                db     $06,$60,$44,$3E,$00
                db     $06,$5E,$44,$3E,$00
                db     $06,$64,$30,$3E,$00
                db     $06,$5E,$30,$3E,$00
                db     $06,$60,$30,$3E,$00
                db     $06,$5E,$30,$3E,$00
                db     $06,$64,$44,$3E,$00
                db     $06,$5E,$44,$3E,$00
                db     $06,$60,$42,$3A,$00
                db     $06,$5E,$42,$3A,$00
                db     $06,$64,$2A,$3A,$00
                db     $06,$5E,$2A,$3A,$00
                db     $06,$60,$2A,$3A,$00
                db     $06,$5E,$2A,$3A,$00
                db     $06,$64,$42,$3A,$00
                db     $06,$5E,$42,$3A,$00
                db     $0C,$60,$3E,$30,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$00,$00,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3E,$2A,$22,$00
                db     $0C,$42,$2A,$22,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3A,$2A,$22,$00
                db     $04,$42,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$42,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3E,$26,$20,$18
                db     $0C,$38,$26,$20,$18
                db     $0C,$30,$26,$20,$18
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$30,$2A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$3E,$2A,$22,$00
                db     $0C,$42,$2A,$22,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $0B,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $05,$48,$26,$20,$18
                db     $01,$00,$00,$00,$00
                db     $06,$44,$26,$20,$18
                db     $06,$42,$26,$20,$18
                db     $06,$3E,$26,$20,$18
                db     $0C,$3A,$2A,$22,$00
                db     $0C,$30,$2A,$22,$00
                db     $0C,$3A,$2A,$22,$00
                db     $05,$30,$00,$00,$48
                db     $02,$00,$00,$00,$00
                db     $05,$30,$00,$00,$48
                db     $0C,$48,$22,$12,$0A
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$52,$3A,$2A,$22
                db     $06,$52,$3A,$30,$00
                db     $06,$56,$3E,$2A,$22
                db     $06,$56,$3E,$30,$00
                db     $06,$5A,$42,$2A,$22
                db     $06,$5A,$42,$30,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$52,$3A,$2A,$22
                db     $06,$52,$3A,$30,$00
                db     $06,$5A,$42,$2A,$22
                db     $06,$5A,$42,$30,$00
                db     $06,$56,$26,$20,$22
                db     $06,$56,$30,$00,$00
                db     $06,$50,$26,$20,$18
                db     $06,$50,$30,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$30,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$48,$2A,$22,$00
                db     $06,$48,$30,$00,$00
                db     $06,$52,$3A,$2A,$22
                db     $06,$52,$3A,$30,$00
                db     $06,$56,$3E,$2A,$22
                db     $06,$56,$3E,$30,$00
                db     $06,$5A,$42,$2A,$22
                db     $06,$5A,$42,$30,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$42,$30,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$30,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$48,$30,$00,$00
                db     $06,$48,$26,$20,$18
                db     $06,$44,$30,$00,$00
                db     $06,$42,$26,$20,$18
                db     $06,$3E,$30,$00,$00
                db     $06,$3A,$2A,$22,$00
                db     $06,$3E,$2A,$22,$00
                db     $06,$42,$2A,$22,$00
                db     $06,$44,$2A,$22,$3A
                db     $06,$42,$2A,$22,$3A
                db     $06,$44,$2A,$22,$3A
                db     $06,$48,$2A,$22,$3A
                db     $06,$4C,$2A,$22,$3A
                db     $06,$48,$2A,$22,$3A
                db     $06,$4C,$2A,$22,$3A
                db     $06,$50,$30,$00,$00
                db     $06,$52,$30,$00,$00
                db     $06,$50,$2A,$00,$00
                db     $06,$52,$2A,$00,$00
                db     $06,$56,$22,$00,$00
                db     $06,$5A,$22,$00,$00
                db     $0C,$5C,$56,$18,$00
                db     $0C,$5C,$56,$1A,$00
                db     $0C,$48,$00,$1C,$00
                db     $0C,$48,$00,$1E,$00
                db     $0C,$5C,$56,$20,$00
                db     $0C,$5C,$56,$26,$00
                db     $0C,$48,$00,$30,$00
                db     $0C,$48,$00,$18,$00
                db     $06,$3A,$22,$00,$00
                db     $06,$3E,$22,$00,$00
                db     $06,$42,$22,$00,$00
                db     $06,$44,$3A,$2A,$22
                db     $06,$42,$3A,$2A,$22
                db     $06,$44,$3A,$2A,$22
                db     $06,$48,$3A,$2A,$22
                db     $06,$4C,$3A,$2A,$22
                db     $06,$48,$2A,$22,$3A
                db     $06,$4C,$2A,$22,$3A
                db     $06,$50,$30,$00,$00
                db     $06,$52,$30,$00,$00
                db     $06,$50,$2A,$00,$00
                db     $06,$52,$2A,$00,$00
                db     $06,$56,$22,$00,$00
                db     $06,$5A,$22,$00,$00
                db     $0C,$5C,$56,$18,$00
                db     $0C,$5C,$56,$1A,$00
                db     $0C,$48,$00,$1C,$00
                db     $0C,$48,$00,$1E,$00
                db     $0C,$5C,$56,$20,$00
                db     $0C,$5C,$56,$26,$00
                db     $0C,$48,$00,$30,$00
                db     $0C,$48,$00,$18,$00
                db     $06,$30,$2A,$22,$00
                db     $06,$30,$2A,$22,$42
                db     $06,$5A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$00,$2A,$1A,$12
                db     $06,$38,$2A,$1A,$12
                db     $06,$50,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $06,$1C,$2A,$22,$00
                db     $06,$1C,$2A,$22,$00
                db     $06,$52,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$00,$30,$26,$18
                db     $06,$44,$30,$26,$18
                db     $06,$5C,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$30,$2A,$22,$00
                db     $06,$30,$2A,$22,$42
                db     $06,$5A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$00,$2A,$1A,$12
                db     $06,$38,$2A,$1A,$12
                db     $06,$50,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $06,$1C,$2A,$22,$00
                db     $06,$1C,$2A,$22,$3A
                db     $06,$52,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$00,$30,$26,$18
                db     $06,$44,$30,$26,$18
                db     $06,$5C,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $0C,$5A,$32,$28,$22
                db     $0C,$52,$00,$00,$00
                db     $0C,$4A,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0B,$5A,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0B,$5A,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0B,$42,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0B,$42,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0C,$5A,$30,$2A,$22
                db     $0C,$52,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0B,$5A,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0B,$5A,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0B,$3C,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0B,$3C,$26,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0C,$5A,$30,$24,$18
                db     $0C,$52,$00,$00,$00
                db     $0C,$4C,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0B,$5E,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0B,$5E,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0B,$46,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0B,$46,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0C,$5E,$28,$22,$1A
                db     $0C,$58,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0C,$58,$00,$00,$00
                db     $0C,$60,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$5E,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$5C,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$5A,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$56,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$52,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$50,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$4C,$30,$2A,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$48,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$46,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$44,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$42,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$3E,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$3A,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$38,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$3E,$30,$2C,$18
                db     $06,$3A,$30,$2A,$22
                db     $06,$3E,$30,$2A,$22
                db     $06,$42,$30,$2A,$22
                db     $06,$44,$3A,$2A,$22
                db     $06,$42,$3A,$2A,$22
                db     $06,$44,$3A,$2A,$22
                db     $06,$48,$3A,$2A,$22
                db     $06,$4C,$3A,$2A,$22
                db     $06,$48,$3A,$2A,$22
                db     $06,$4C,$3A,$2A,$22
                db     $06,$50,$30,$00,$00
                db     $06,$52,$30,$00,$00
                db     $06,$50,$2A,$00,$00
                db     $06,$52,$2A,$00,$00
                db     $06,$56,$22,$00,$00
                db     $06,$5A,$22,$00,$00
                db     $0C,$5C,$56,$18,$00
                db     $0C,$5C,$56,$1A,$00
                db     $0C,$48,$00,$1C,$00
                db     $0C,$48,$00,$1E,$00
                db     $0C,$5C,$56,$20,$00
                db     $0C,$5C,$56,$26,$00
                db     $0C,$48,$00,$30,$00
                db     $0C,$48,$00,$18,$00
                db     $06,$3A,$30,$00,$00
                db     $06,$3E,$30,$00,$00
                db     $06,$42,$30,$00,$00
                db     $06,$44,$3A,$2A,$22
                db     $06,$42,$3A,$2A,$22
                db     $06,$44,$3A,$2A,$22
                db     $06,$48,$3A,$2A,$22
                db     $06,$4C,$3A,$2A,$22
                db     $06,$48,$3A,$2A,$22
                db     $06,$4C,$3A,$2A,$22
                db     $06,$50,$30,$00,$00
                db     $06,$52,$30,$00,$00
                db     $06,$50,$2A,$00,$00
                db     $06,$52,$2A,$00,$00
                db     $06,$56,$22,$00,$00
                db     $06,$5A,$22,$00,$00
                db     $0C,$5C,$56,$18,$00
                db     $0C,$5C,$56,$1A,$00
                db     $0C,$48,$00,$1C,$00
                db     $0C,$48,$00,$1E,$00
                db     $0C,$5C,$56,$20,$00
                db     $0C,$5C,$56,$26,$00
                db     $0C,$48,$00,$30,$00
                db     $0C,$48,$00,$18,$00
                db     $06,$00,$30,$2A,$22
                db     $06,$42,$30,$2A,$22
                db     $06,$5A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$00,$2A,$12,$1A
                db     $06,$38,$2A,$12,$1A
                db     $06,$50,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $06,$00,$2A,$22,$1C
                db     $06,$3A,$2A,$22,$1C
                db     $06,$52,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$00,$30,$26,$18
                db     $06,$44,$30,$26,$18
                db     $06,$5C,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$00,$30,$2A,$22
                db     $06,$42,$30,$2A,$22
                db     $06,$5A,$00,$00,$00
                db     $06,$42,$00,$00,$00
                db     $06,$00,$2A,$12,$1A
                db     $06,$38,$2A,$12,$1A
                db     $06,$50,$00,$00,$00
                db     $06,$38,$00,$00,$00
                db     $06,$00,$2A,$22,$1C
                db     $06,$3A,$2A,$22,$1C
                db     $06,$52,$00,$00,$00
                db     $06,$3A,$00,$00,$00
                db     $06,$00,$30,$26,$18
                db     $06,$44,$30,$26,$18
                db     $06,$5C,$00,$00,$00
                db     $06,$44,$00,$00,$00
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $06,$5A,$32,$28,$22
                db     $06,$42,$32,$28,$22
                db     $0C,$5A,$32,$28,$22
                db     $0C,$52,$00,$00,$00
                db     $0C,$4A,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0B,$5A,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0B,$5A,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0B,$42,$30,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0B,$42,$26,$2A,$22
                db     $01,$00,$00,$00,$00
                db     $0C,$5A,$30,$2A,$22
                db     $0C,$52,$00,$00,$00
                db     $0C,$48,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0B,$5A,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0B,$5A,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0B,$3C,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0B,$3C,$30,$24,$1C
                db     $01,$00,$00,$00,$00
                db     $0C,$5A,$30,$24,$1C
                db     $0C,$54,$00,$00,$00
                db     $0C,$4E,$00,$00,$00
                db     $0C,$54,$00,$00,$00
                db     $0B,$5E,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0B,$5E,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0B,$46,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0B,$46,$28,$22,$1A
                db     $01,$00,$00,$00,$00
                db     $0C,$5E,$28,$22,$1A
                db     $0C,$5A,$00,$00,$00
                db     $0C,$52,$00,$00,$00
                db     $0C,$5A,$00,$00,$00
                db     $0C,$60,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$5E,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$5C,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$5A,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$56,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$52,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$50,$30,$2A,$18
                db     $06,$00,$30,$2A,$18
                db     $06,$4C,$30,$2A,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$48,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$46,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$44,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$42,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$3E,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$3A,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$38,$30,$2C,$18
                db     $06,$00,$30,$2C,$18
                db     $06,$34,$30,$2C,$18
                db     $06,$3A,$30,$2A,$22
                db     $06,$38,$30,$2A,$22
                db     $06,$3A,$30,$2A,$22
                db     $06,$38,$30,$2A,$22
                db     $0C,$3A,$30,$2A,$22
                db     $05,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $0A,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $0A,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $0C,$52,$3A,$00,$00
                db     $0C,$56,$3E,$00,$00
                db     $0C,$5A,$42,$00,$00
                db     $05,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $0A,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $0A,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $02,$00,$00,$00,$00
                db     $05,$48,$30,$00,$00
                db     $0C,$5A,$42,$00,$00
                db     $0C,$5C,$44,$00,$00
                db     $0C,$60,$48,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $0C,$52,$3A,$00,$00
                db     $0C,$56,$3E,$00,$00
                db     $0C,$5A,$42,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $0C,$5A,$42,$2A,$00
                db     $0C,$5C,$44,$2C,$00
                db     $0C,$60,$48,$30,$00
                db     $0C,$00,$00,$00,$00
                db     $30,$00,$00,$00,$00
                db     $06,$48,$2C,$26,$18
                db     $06,$4A,$2C,$26,$18
                db     $06,$4C,$2C,$26,$18
                db     $06,$4E,$2C,$26,$18
                db     $06,$50,$2C,$26,$18
                db     $06,$52,$2C,$26,$18
                db     $06,$54,$2C,$26,$18
                db     $06,$56,$2C,$26,$18
                db     $06,$58,$2C,$26,$18
                db     $06,$5A,$2C,$26,$18
                db     $06,$5C,$2C,$26,$18
                db     $06,$5E,$2C,$26,$18
                db     $06,$60,$2C,$26,$18
                db     $06,$62,$2C,$26,$18
                db     $06,$64,$2C,$26,$18
                db     $06,$68,$2C,$26,$18
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $06,$6A,$2A,$22,$00
                db     $06,$68,$2A,$22,$00
                db     $06,$6E,$2C,$18,$00
                db     $06,$68,$2C,$18,$00
                db     $0C,$6A,$2A,$22,$00
                db     $06,$6A,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$6A,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$6A,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $0C,$6A,$30,$2A,$22
                db     $06,$60,$30,$2A,$22
                db     $06,$48,$30,$2A,$22
                db     $06,$60,$30,$2A,$22
                db     $06,$48,$30,$2A,$22
                db     $06,$60,$30,$2A,$22
                db     $06,$48,$30,$2A,$22
                db     $0C,$60,$30,$2A,$22
                db     $06,$5A,$30,$2A,$22
                db     $06,$42,$30,$2A,$22
                db     $06,$5A,$30,$2A,$22
                db     $06,$42,$30,$2A,$22
                db     $06,$5A,$30,$2A,$22
                db     $06,$42,$30,$2A,$22
                db     $0C,$5A,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$3A,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$3A,$30,$2A,$22
                db     $06,$52,$30,$2A,$22
                db     $06,$3A,$30,$2A,$22
                db     $0C,$52,$30,$2A,$22
                db     $04,$52,$3A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $04,$52,$3A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$52,$3A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$52,$3A,$22,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$52,$3A,$22,$00
                db     $04,$48,$30,$18,$00
                db     $02,$00,$00,$00,$00
                db     $04,$48,$30,$18,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$30,$18,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$48,$30,$00,$18
                db     $02,$00,$00,$00,$00
                db     $0C,$48,$30,$18,$00
                db     $04,$42,$2A,$12,$00
                db     $02,$00,$00,$00,$00
                db     $04,$42,$2A,$12,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$42,$2A,$12,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$42,$2A,$12,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$42,$2A,$12,$00
                db     $04,$3A,$22,$0A,$00
                db     $02,$00,$00,$00,$00
                db     $04,$3A,$22,$0A,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$3A,$22,$0A,$00
                db     $02,$00,$00,$00,$00
                db     $0A,$3A,$22,$0A,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$3A,$22,$0A,$00
                db     $0C,$30,$18,$00,$00
                db     $0C,$3A,$22,$00,$00
                db     $0C,$30,$18,$00,$00
                db     $0C,$42,$2A,$00,$00
                db     $0C,$3A,$22,$00,$00
                db     $0C,$48,$30,$00,$00
                db     $0C,$42,$2A,$00,$00
                db     $0C,$52,$3A,$00,$00
                db     $0C,$48,$18,$00,$00
                db     $0C,$52,$22,$00,$00
                db     $0C,$48,$18,$00,$00
                db     $0C,$5A,$2A,$00,$00
                db     $0C,$52,$22,$00,$00
                db     $0C,$60,$30,$00,$00
                db     $0C,$5A,$2A,$00,$00
                db     $18,$52,$3A,$00,$00
                db     $0C,$00,$00,$00,$00
                db     $04,$5A,$52,$3A,$00
                db     $02,$00,$00,$00,$00
                db     $04,$5A,$52,$3A,$00
                db     $02,$00,$00,$00,$00
                db     $0C,$5A,$52,$22,$00
                db     $0C,$5A,$52,$30,$00
                db     $0C,$5A,$52,$2A,$00
                db     $0C,$5A,$52,$30,$00
                db     $18,$5A,$52,$22,$00
                db     $12,$00,$00,$00,$00
                db     $04,$5A,$52,$22,$30
                db     $02,$00,$00,$00,$00
                db     $18,$5A,$52,$22,$30
                db     $12,$00,$00,$00,$00
                db     $04,$3A,$2A,$18,$0A
                db     $02,$00,$00,$00,$00
                db     $30,$3A,$2A,$18,$0A
;terminate the song.  At most, it *should* only require 5 $00 bytes to stop. (duration and 4 notes)
                db     $00,$00,$00,$00,$00
	
	
	end
